...
void ConfigService::CreateConfigService(Json::Value &json) {
    EmxPlatform::PlatE pe;
    auto &config = json["config"];
    auto &factory = json["factory"];
    ...
    pe = EmxPlatform::GetPlat(factory["platformType"].asString());
    ...
    Create(m_loop, "0.0.0.0",
           ...
           m_buffer,
           m_bufferSize);
    m_protoV1.SetPlatform(pe);
    m_protoV1.BindLoop(&m_loop);
    m_loop.Start();
}
