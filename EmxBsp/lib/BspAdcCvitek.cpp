...
BspAdcCvitek::BspAdcCvitek() : Bsp::Adc() {
    m_created = false;
...
BspAdcCvitek::~BspAdcCvitek() {
    m_channels.clear();
}

int BspAdcCvitek::GetLumaValue(int chn) {
    if (!m_created)
        return -1;
...
int BspAdcCvitek::GetAdcValue(int chn) {
    int val = -1;
    auto inst = BspUtils::GetInst();
    inst->Lock();
    do {
        ...
        snprintf(path, sizeof(path),
                 "/sys/class/cvi-saradc/cvi-saradc%d/device/cv_saradc",
                 m_channels[chn].chnNum);
