...
int32_t ThirdpartyAudioDevice::InitPlayout() {
  ...
  if (_ptrAudioBuffer)
  {
      ...
      _ptrAudioBuffer->SetPlayoutSampleRate(voip_info_t->audio_samplerate);
      _ptrAudioBuffer->SetPlayoutChannels(voip_info_t->audio_chn_num);
  }
  return 0;
}
...
int32_t ThirdpartyAudioDevice::InitRecording() {
  CriticalSectionScoped lock(&_critSect);
    ...
    cmcc_voip_info_t *voip_info_t = get_cmcc_voip_info();
    _recordingFramesIn10MS = voip_info_t->audio_samplerate/100;

    if (_ptrAudioBuffer) {
    _ptrAudioBuffer->SetRecordingSampleRate(voip_info_t->audio_samplerate);
    _ptrAudioBuffer->SetRecordingChannels(voip_info_t->audio_chn_num);
    }
    return 0;
}
...
int32_t ThirdpartyAudioDevice::StartPlayout() {
  ...
  _playing = true;
  _playoutFramesLeft = 0;
  _playoutFramesIn10MS = voip_info_t->audio_samplerate/100;

  _playingRectifier.ClearStatistics(); // 播放累积统计重置

  if (!_playoutBuffer)
      _playoutBuffer = new int8_t[2 * voip_info_t->audio_chn_num *
                                    voip_info_t->audio_samplerate/100];
