...
ErrCodeE AlarmInvasion::Create() {
    ...
    m_ai->Start(nullptr, [this](MediaAi::AiInfoHeader &header) {
        ...
        for (int i = 0; i < header.size / (int) sizeof(MediaAi::AiInfoClassifyData); i++) {
            ...
            if (!m_ctx->env.invasion.ena) {
                IntelligentLight(&m_ctx->env.invasion.ena, 15);
                return;
            }
            if (AlarmInZone(header.srcPicSize, data[i].rect, m_ctx->env.invasion.zone)) {
                bool started = m_started;
                AlarmTrigger();
                //触发开始
                if (m_started && !started) {
                    Strategy(&m_ctx->env.invasion.strategy, &m_ctx->env.invasion.ena, "");
                }
            } else {
                IntelligentLight(&m_ctx->env.invasion.ena, 15);
            }
