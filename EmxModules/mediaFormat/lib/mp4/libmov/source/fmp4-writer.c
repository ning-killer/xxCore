...
static int fmp4_write_fragment(struct fmp4_writer_t* writer)
{
	...
	while(n < writer->mdat_size)
	{
		for (i = 0; i < mov->track_count; i++)
		{
            ...
            {
			    printf("%s\n", mov->track->handler_type == MOV_VIDEO ? "Video":"Audio");
...
//		mov->ftyp.compatible_brands[5] = MOV_BRAND_ISO6; // styp
        mov->ftyp.compatible_brands[0] = MOV_BRAND_ISOM;
        mov->ftyp.compatible_brands[1] = MOV_BRAND_MP41;
        mov->ftyp.compatible_brands[2] = MOV_BRAND_ISO2; // default�\base�\is�\moof flag
        mov->ftyp.compatible_brands[3] = MOV_BRAND_ISO6; // styp
		mov->header = 0;
	}
	else
	{
