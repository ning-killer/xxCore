...
static int flv_muxer_h264(struct flv_muxer_t *flv, uint32_t pts, uint32_t dts) {
    ...
    if ( /*0 == flv->video_sequence_header &&*/ flv->update && flv->v.avc.nb_sps > 0 && flv->v.avc.nb_pps > 0) {
        ...
        m = mpeg4_avc_decoder_configuration_record_save(&flv->v.avc, flv->ptr + flv->bytes + 5,
                                                        flv->capacity - flv->bytes - 5);
        if (m <= 0)
            return -1; // invalid data
...
int flv_muxer_avc(struct flv_muxer_t *flv, const void *data, size_t bytes, uint32_t pts, uint32_t dts) {
    ...
    flv->bytes = 5;
    flv->bytes += h264_annexbtomp4(&flv->v.avc, data, (int) bytes, flv->ptr + flv->bytes, flv->capacity - flv->bytes,
                                   &flv->vcl, &flv->update);
    if (flv->bytes <= 5)
        return -ENOMEM;
...
static int flv_muxer_h265(struct flv_muxer_t *flv, uint32_t pts, uint32_t dts) {
    ...
    if ( /*0 == flv->avc_sequence_header &&*/ flv->update && flv->v.hevc.numOfArrays >= 3) // vps + sps + pps
    {
        ...
        video.avpacket = FLV_SEQUENCE_HEADER;
        flv_video_tag_header_write(&video, flv->ptr + flv->bytes, flv->capacity - flv->bytes);
        m = mpeg4_hevc_decoder_configuration_record_save(&flv->v.hevc, flv->ptr + flv->bytes + 5,
                                                         flv->capacity - flv->bytes - 5);
        if (m <= 0)
            return -1; // invalid data
...
int flv_muxer_hevc(struct flv_muxer_t *flv, const void *data, size_t bytes, uint32_t pts, uint32_t dts) {
    ...
    flv->bytes = 5;
    flv->bytes += h265_annexbtomp4(&flv->v.hevc, data, (int) bytes, flv->ptr + flv->bytes, flv->capacity - flv->bytes,
                                   &flv->vcl, &flv->update);
    if (flv->bytes <= 5)
        return -ENOMEM;
...
int flv_muxer_av1(flv_muxer_t *flv, const void *data, size_t bytes, uint32_t pts, uint32_t dts) {
    ...
    if (0 == flv->video_sequence_header) {
        ...
        video.avpacket = FLV_SEQUENCE_HEADER;
        flv_video_tag_header_write(&video, flv->ptr + flv->bytes, flv->capacity - flv->bytes);
        m = aom_av1_codec_configuration_record_save(&flv->v.av1, flv->ptr + flv->bytes + 5,
                                                    flv->capacity - flv->bytes - 5);
        if (m <= 0)
            return -1; // invalid data
...
int flv_muxer_metadata(flv_muxer_t *flv, const struct flv_metadata_t *metadata) {
    ...
    if (flv->capacity < (int) sizeof(struct flv_metadata_t) + metadata->user.size) {
        if (0 != flv_muxer_alloc(flv, (int) sizeof(struct flv_metadata_t) + metadata->user.size + 1024))
            return -ENOMEM;
    }
    ptr = flv->ptr;
    ...
    ptr = AMFWriteNamedDouble(ptr, end, "audio.codec", 11, metadata->audio.codec);
    ptr = AMFWriteNamedDouble(ptr, end, "audio.samplerate", 16, metadata->audio.samplerate /* / 1024.0*/);
    ptr = AMFWriteNamedDouble(ptr, end, "audio.bitwidth", 14, metadata->audio.bitwidth);
    ptr = AMFWriteNamedDouble(ptr, end, "audio.channels", 14, metadata->audio.channels);
    ptr = AMFWriteNamedDouble(ptr, end, "audio.bitrate", 13, metadata->audio.bitrate);
    ptr = AMFWriteNamedDouble(ptr, end, "video.codec", 11, metadata->video.codec);
    ptr = AMFWriteNamedDouble(ptr, end, "video.width", 11, metadata->video.width);
    ptr = AMFWriteNamedDouble(ptr, end, "video.height", 12, metadata->video.height);
    ptr = AMFWriteNamedDouble(ptr, end, "video.fps", 9, metadata->video.fps);
    ptr = AMFWriteNamedDouble(ptr, end, "video.gop", 9, metadata->video.gop);
    ptr = AMFWriteNamedDouble(ptr, end, "duration", 8, metadata->duration);
    if (metadata->user.size) {
        ptr = AMFWriteNamedString(ptr, end, "userdata", 8, metadata->user.data, metadata->user.size);
    }
    ptr = AMFWriteObjectEnd(ptr, end);
    int k = flv->handler(flv->param, FLV_TYPE_SCRIPT, flv->ptr, ptr - flv->ptr, 0);
    return k;
}
