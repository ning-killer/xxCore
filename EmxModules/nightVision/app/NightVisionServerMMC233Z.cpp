...
ErrCodeE NightVisionServerMMC233Z::CreateSub(Json::Value &json) {
    try {
        ...
        if (json["antiShock"]["ir2day"]["enable"].asBool()) {
            m_ir2dayAntiShok.enable = json["antiShock"]["ir2day"]["enable"].asBool();
        }
        if (json["antiShock"]["ir2day"]["unlockTime"].isInt()) {
            m_ir2dayAntiShok.unlockTime = json["antiShock"]["ir2day"]["unlockTime"].asInt();
        }
        if (json["antiShock"]["ir2day"]["oscillationInterval"].isInt()) {
            m_ir2dayAntiShok.oscillationInterval = json["antiShock"]["ir2day"]["oscillationInterval"].asInt();
        }
        if (json["antiShock"]["ir2day"]["switchCount"].isInt()) {
            m_ir2dayAntiShok.switchCount = json["antiShock"]["ir2day"]["switchCount"].asInt();
        }
        m_ir2dayAntiShok.lastlockTime = Time::GetS();
        m_ir2dayAntiShok.countContinueUnlock = 0;
        m_ir2dayAntiShok.islocked = false;
        ...
        m_timer.Start(3000, 120, std::bind(&NightVisionServerMMC233Z::OnAutoModeJudge, this));
...
void NightVisionServerMMC233Z::DestroySub() {
    m_isp.Stop();
    m_timer.Destroy();
}

void NightVisionServerMMC233Z::PublishReset() {
    m_ir2dayAntiShok.switchCaches.clear();
    m_ir2dayAntiShok.lastlockTime = Time::GetS();
    m_ir2dayAntiShok.countContinueUnlock = 0;
    m_ir2dayAntiShok.islocked = false;
}

void NightVisionServerMMC233Z::DoSwitch(NightVision::ModeE mode) {
    switch (mode) {
        case ModeE::Day: {
            for (auto &chn :m_cfg.chn) {
                m_bsp.irCut->SwitchToNight(false, chn.irCut);
                // m_bsp.irLight->SetEna(false, chn.irLight);
                m_bsp.whiteLight->SetEna(false, chn.irLight);
        ...
        case ModeE::IrNightVision: {
            for (auto &chn :m_cfg.chn) {
                ...
                m_bsp.whiteLight->SetEna(true, chn.irLight);
                emxlogd("toirDelay[%d]\n", m_toirDelay);
...
void NightVisionServerMMC233Z::OnAutoModeJudge() {
    if (m_running.manual) {
        m_debounceCount = 0;
        return;
    }
...
void NightVisionServerMMC233Z::Day2IrJudge() {
    // day -> IR night
    ...
    if (m_debounceCount >= 5) {
        m_debounceCount = 0;
        AntiShockJudge(AntiShockStatus::ToIr);
        Switch(NightVision::ModeE::IrNightVision);
    }
}

void NightVisionServerMMC233Z::Ir2DayJudge() {
    // IR night -> day
    if (m_cfg.ir2dayUsingHard) { // 使用硬件光敏
        ...
        if (m_ispInfo.blueGain > soft.iso &&
            m_ispInfo.iso < soft.iso &&
            m_ispInfo.luma < 3072) {
            if ((m_ispInfo.blueGain - m_ispInfo.redGain) > 400000 &&
                (m_ispInfo.blueGain - m_ispInfo.redGain) < 550000 &&
                (m_ispInfo.blueGain + m_ispInfo.redGain > 500000)) {
                // 为反光场景, 不切day
                m_debounceCount = 0;
                return;
            }
    ...
    emxlogt("m_debounceCount[%d]\n", m_debounceCount);
    if (m_debounceCount >= 5) {
        m_debounceCount = 0;
        AntiShockJudge(AntiShockStatus::ToDay);
        if (!m_ir2dayAntiShok.islocked) {
            Switch(NightVision::ModeE::Day);
        }
    }
}

int NightVisionServerMMC233Z::GetLockTimer() {
    int locktime;
    if (m_ir2dayAntiShok.countContinueUnlock == 1) {
        locktime = 10 * 60;
    } else if (m_ir2dayAntiShok.countContinueUnlock == 2) {
        locktime = 20 * 60;
    } else if (m_ir2dayAntiShok.countContinueUnlock == 3) {
        locktime = 30 * 60;
    } else {
        locktime = 60 * 60;
    }
...
void NightVisionServerMMC233Z::AntiShockJudge(AntiShockStatus status) {
    if (m_ir2dayAntiShok.enable && !m_param.manual
        && m_param.autoMode == NightVision::AutoModeE::IrNightVision) {
        int currentTime = Time::GetS();
        if (m_ir2dayAntiShok.islocked) {
            if ((currentTime - m_ir2dayAntiShok.lastlockTime) > m_ir2dayAntiShok.timer) {
                m_ir2dayAntiShok.islocked = false;
                for (auto &cache : m_ir2dayAntiShok.switchCaches) {
                    cache += m_ir2dayAntiShok.timer;
                }
            }
            emxlogt("spent timer[%d]<--->locktime[%d]\n"
            , currentTime - m_ir2dayAntiShok.lastlockTime
            , m_ir2dayAntiShok.timer);
            return;
        }
        if ((int)m_ir2dayAntiShok.switchCaches.size() < m_ir2dayAntiShok.switchCount) {
            m_ir2dayAntiShok.switchCaches.emplace_back(currentTime);
        } else {
            m_ir2dayAntiShok.switchCaches.erase(m_ir2dayAntiShok.switchCaches.begin());
            m_ir2dayAntiShok.switchCaches.shrink_to_fit();
            m_ir2dayAntiShok.switchCaches.emplace_back(currentTime);
        }
        if (m_ir2dayAntiShok.islocked) {
            return;
        }
        emxlogt("cache size[%d]\n", (int)m_ir2dayAntiShok.switchCaches.size());
        if ((int)m_ir2dayAntiShok.switchCaches.size() == m_ir2dayAntiShok.switchCount) {
            emxlogt("[%d]<--->[%d]\n", m_ir2dayAntiShok.switchCaches[m_ir2dayAntiShok.switchCount - 1], m_ir2dayAntiShok.switchCaches[0]);
            if (m_ir2dayAntiShok.switchCaches[m_ir2dayAntiShok.switchCount - 1] - m_ir2dayAntiShok.switchCaches[0]
                < m_ir2dayAntiShok.oscillationInterval) {
                m_ir2dayAntiShok.islocked = true;
                m_ir2dayAntiShok.lastlockTime = currentTime;
                m_ir2dayAntiShok.countContinueUnlock++;
                emxlogt("countContinueUnlock[%d]\n", m_ir2dayAntiShok.countContinueUnlock);
                m_ir2dayAntiShok.timer = GetLockTimer();
            } else {
                m_ir2dayAntiShok.countContinueUnlock = 0;
            }
            emxlogt("[%d]count spent time[%d]<--->configtimer[%d]\n"
            , m_ir2dayAntiShok.switchCount
            , m_ir2dayAntiShok.switchCaches[m_ir2dayAntiShok.switchCount - 1] - m_ir2dayAntiShok.switchCaches[0]
            , m_ir2dayAntiShok.oscillationInterval);
        }
    }
...
void NightVisionServerMMC233Z::Day2ColorJudge() {
    // day -> color night
...
void NightVisionServerMMC233Z::Color2DayJudge() {
    // color night -> day
