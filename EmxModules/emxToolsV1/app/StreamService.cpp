...
void StreamService::OnRecv(EuvStmServerClientBase *client, char *data, int32_t size) {
    ...
    while (c->data.size() >= c->waitSize) {
        ...
        if ((info.chn >= 2) ||
            ...
            videoInfo.errorCode = htonl((uint32_t) Stream::ErrCodeE::UnSupportStreamType);
...
//#define PRINT_MEDIA_DELAY

#ifdef PRINT_MEDIA_DELAY
static const int MaxRecTime = 64;
static uint64_t timeW[MaxRecTime];
static int idxW = 0, idxR = 0;
#endif
...
#ifdef PRINT_MEDIA_DELAY
    emxlogn("SendDoneClock-PtsClock=%llums\n", (Time::GetMonoClockUs() - timeW[idxR++]) / 1000);
    if (idxR == MaxRecTime)idxR = 0;
#endif
}
...
void StreamClient::OnGetFrame(ErrCodeE e, MediaFrame &frame) {
    ...
    if (frame.type == MediaFrame::TypeE::Venc) {
        ...
        video.frameType = frame.isKeyFrame ? Stream::FrameTypeE::IFrame : Stream::FrameTypeE::PFrame;
    ...
    buffered++;
#ifdef PRINT_MEDIA_DELAY
    timeW[idxW++] = frame.tsInUs;
    if (idxW == MaxRecTime)idxW = 0;
#endif
...
void StreamClient::SetupStream(int chn) {
    ...
    if (chn == 0) {
        if (vParam.codec == VideoCodecE::H264) {
            video.encoderType = Stream::VideoCodecChnTypeE::H264Major;
        } else if (vParam.codec == VideoCodecE::H265) {
            video.encoderType = Stream::VideoCodecChnTypeE::H265Major;
        } else if (vParam.codec == VideoCodecE::MJPEG) {
            video.encoderType = Stream::VideoCodecChnTypeE::MjpegMajor;
        }
    } else {
        if (vParam.codec == VideoCodecE::H264) {
            video.encoderType = Stream::VideoCodecChnTypeE::H264Minor;
        } else if (vParam.codec == VideoCodecE::H265) {
            video.encoderType = Stream::VideoCodecChnTypeE::H265Minor;
        } else if (vParam.codec == VideoCodecE::MJPEG) {
            video.encoderType = Stream::VideoCodecChnTypeE::MjpegMinor;
        }
    }
