...
static char *default_environment = NULL;
static size_t default_environment_size = 0;
//extern char *g_bootEnvCfgDir;
extern  char g_bootEnvCfgDir[256];
...
int load_default_environment() {
    FILE *fp = NULL;
    char line[1024];
    size_t str_len = 0;
    default_environment = (char *) calloc(1, CUR_ENVSIZE);
    if (!default_environment) {
        printf("malloc default environment failed, size:%d\n", (int) CUR_ENVSIZE);
        goto ERR;
    }
    default_environment_size = 0;
    char path[256];
    snprintf(path, sizeof(path), "%s/%s", g_bootEnvCfgDir, DEFAULT_FILE);
    fp = fopen(path, "rt");
    if (!fp) {
    ...
    while (fgets(line, 1024, fp)) {
        str_len = strlen(line);
        int i;
        for (i = (int) str_len - 1; i >= 0 && (line[i] == '\r' || line[i] == '\n'); i--, str_len--);
        line[str_len++] = '\0';
        if (default_environment_size + str_len > CUR_ENVSIZE) {
            printf("default_environment_size oversize %d > %d\n",
                   (int) (default_environment_size + str_len),
                   (int) CUR_ENVSIZE);
            goto ERR;
        }
        memcpy(default_environment + default_environment_size,
               line, str_len);
        default_environment_size += str_len;
    }
    default_environment[default_environment_size++] = '\0';
    fclose(fp);
    return 0;
    ERR:
    if (default_environment)
        free(default_environment);
    default_environment_size = 0;
    if (fp)
        fclose(fp);
    return -1;
...
int fw_env_open(struct env_opts *opts) {
    ...
    if (parse_config(opts))        /* should fill envdevices */
        return -EINVAL;
    if (load_default_environment()) {
        printf("fw_env_open cannot load default environment\n");
        return -EINVAL;
    }
...
static int get_config(char *fname) {
    ...
    char *devname;
    char path[256];
    snprintf(path, sizeof(path), "%s/%s", g_bootEnvCfgDir, fname);
    fp = fopen(path, "r");
    if (fp == NULL)
        return -1;
...
int get_env(char **p, int *max_size) {
    struct env_opts *opts = &default_opts;
    if (fw_env_open(opts)) {
        return -1;
    }
    *p = calloc(1, ENV_SIZE);
    ...
    memcpy(*p, environment.data, ENV_SIZE);
    *max_size = (int) ENV_SIZE;
    fw_env_close(opts);
    return 0;
}
