...
void aes_decrypt(uint8_t *in, uint8_t *expkey, uint8_t *out) {
    ...
    add_round_key((uint32_t *) state,
                  (uint32_t *) expkey + AES_ROUNDS * AES_STATECOLS);
...
static void debug_print_vector(char *name, uint32_t num_bytes, uint8_t *data) {
#ifdef DEBUG
    printf("%s [%d] @0x%08x", name, num_bytes, (uint32_t)data);
...
void aes_cbc_encrypt_blocks(uint8_t *key_exp, uint8_t *src, uint8_t *dst, uint32_t num_aes_blocks) {
    uint8_t zero_key[AES_KEY_LENGTH] = {0};
    uint8_t tmp_data[AES_KEY_LENGTH];
    /* Convenient array of 0's for IV */
    uint8_t *cbc_chain_data = zero_key;
    uint32_t i;
...
void aes_cbc_decrypt_blocks(uint8_t *key_exp, uint8_t *src, uint8_t *dst, uint32_t num_aes_blocks) {
    uint8_t tmp_data[AES_KEY_LENGTH], tmp_block[AES_KEY_LENGTH];
    /* Convenient array of 0's for IV */
    uint8_t cbc_chain_data[AES_KEY_LENGTH] = {0};
    uint32_t i;
