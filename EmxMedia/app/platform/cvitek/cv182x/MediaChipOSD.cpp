...
// bool MediaChipOSD::ParamFastConfig(const std::vector<std::shared_ptr<Param>> &param) {
//     emxlogd("MediaChipOSD::ParamFastConfig start.\n");
//     for (int i = 0; i < (int) param.size(); i++) {
//         if (param[i]->type != m_param[i]->type)
//             return false;
//         if (param[i]->type == MediaOSD::TypeE::Time) {
//             auto ena = param[i]->time.ena;
//             param[i]->time.ena = m_param[i]->time.ena;
//             auto same = memcmp(&param[i]->time, &m_param[i]->time, sizeof(param[i]->time));
//             param[i]->time.ena = ena;
//             if (!same)
//                 return false;
//         } else if (param[i]->type == MediaOSD::TypeE::Text) {
//             auto ena = param[i]->text.ena;
//             param[i]->text.ena = m_param[i]->text.ena;
//             auto same = memcmp(&param[i]->text, &m_param[i]->text, sizeof(param[i]->text));
//             param[i]->text.ena = ena;
//             if (!same)
//                 return false;
//         } else if (param[i]->type == MediaOSD::TypeE::Image) {
//             auto ena = param[i]->image.ena;
//             param[i]->image.ena = m_param[i]->image.ena;
//             auto same = memcmp(&param[i]->image, &m_param[i]->image, sizeof(param[i]->image));
//             param[i]->image.ena = ena;
//             if (!same)
//                 return false;
//         } else {
//             return false;
...
//     for (int i = 0; i < (int) param.size(); i++) {
//         if (param[i]->type == MediaOSD::TypeE::Time) {
//             m_param[i]->time.ena = param[i]->time.ena;
//         } else if (param[i]->type == MediaOSD::TypeE::Text) {
//             m_param[i]->text.ena = param[i]->time.ena;
//         } else if (param[i]->type == MediaOSD::TypeE::Image) {
//             m_param[i]->image.ena = param[i]->image.ena;
//         }
//     }

//     emxlogd("ParamFastConfig RegionShow start.\n");
//     for (auto &e : m_osdTime) {
//         RegionShow(e.handle, e.time->ena);
//     }
//     for (auto &e : m_osdText) {
//         RegionShow(e.handle, e.text->ena);
//     }
//     for (auto &e : m_osdImage) {
//         RegionShow(e.handle, e.image->ena);
//     }
//     SaveParam();
//     return true;
// }

// ErrCodeE MediaChipOSD::RegionShow(RGN_HANDLE handle, bool show) {
//     CVI_S32 s32Ret;
//     MMF_CHN_S stChn = {
//             .enModId = CVI_ID_VPSS,
//             .s32DevId = m_vpssGrp,
...
//     };
//     RGN_CHN_ATTR_S stChnAttr = {};
//     s32Ret = CVI_RGN_GetDisplayAttr(handle, &stChn, &stChnAttr);
//     if (s32Ret != CVI_SUCCESS) {
//         emxloge("chn %d CVI_RGN_GetDisplayAttr %d failed! s32Ret: 0x%x.\n", m_chn, handle, s32Ret);
//         return ErrCodeE::Failure;
//     }

//     stChnAttr.bShow = (CVI_BOOL) show;

//     s32Ret = CVI_RGN_SetDisplayAttr(handle, &stChn, &stChnAttr);
//     if (s32Ret != CVI_SUCCESS) {
//         emxloge("chn %d CVI_RGN_SetDisplayAttr %d failed! s32Ret: 0x%x.\n", m_chn, handle, s32Ret);
//         return ErrCodeE::Failure;
//     }
//     return ErrCodeE::Success;
// }
