...
namespace Emx {
    ...
    protected:
        ...
        virtual ErrCodeE GetVqeParam(AO_VQE_CONFIG_S &vqe);

        virtual void OnGetMsg(uint32_t topic, const char *data, int32_t size);

        virtual void PlayFile(void *arg);

        virtual void PlayFileDone(ErrCodeE e, void *arg);

        virtual void RunPlayWork();

        virtual void SendToAo(const int16_t *pcm, int32_t pktNum);

        virtual void SpkEna(bool ena);
