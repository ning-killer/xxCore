...
ErrCodeE MediaChipSystem::InitVi(const Json::Value &json) {
    ...
    CVI_S32 s32Ret;
    std::vector<VI_DEV> vViDev;
    char path[EMX_MAX_PATH_SIZE] = {};
    memset(&m_viCfg, 0, sizeof(m_viCfg));
    ...
    if (!SAMPLE_COMM_VI_ParseIni(&stIniCfg)) {
        emxloge("SAMPLE_COMM_VI_ParseIni failed!\n");
        return ErrCodeE::Failure;
    }
    ...
    if (s32Ret != CVI_SUCCESS) {
        emxloge("SAMPLE_COMM_VI_IniToViCfg failed with %#x\n", s32Ret);
        return ErrCodeE::Failure;
    }

    s32Ret = SAMPLE_COMM_VI_StartSensor(&m_viCfg);
    if (s32Ret != CVI_SUCCESS) {
        emxloge("system start sensor failed with %#x\n", s32Ret);
        return ErrCodeE::Failure;
    }
    ...
    for (VI_DEV ViDev = 0; ViDev < m_viCfg.s32WorkingViNum; ViDev++) {
        ...
        if (s32Ret != CVI_SUCCESS) {
            emxloge("VI_StartDev[%d] failed with %#x\n", ViDev, s32Ret);
    ...
    if (s32Ret != CVI_SUCCESS) {
        emxloge("system start MIPI failed with %#x\n", s32Ret);
        goto EXIT_DESTROY_DEV;
    }

    s32Ret = SAMPLE_COMM_VI_SensorProbe(&m_viCfg);
    if (s32Ret != CVI_SUCCESS) {
        emxloge("system sensor probe failed with %#x\n", s32Ret);
        goto EXIT_DESTROY_DEV;;
    }
    ...
    if (s32Ret != CVI_SUCCESS) {
        emxloge("VI_CreateIsp failed with %#x!\n", s32Ret);
        goto EXIT_DESTROY_PIPE;
    }

    s32Ret = SAMPLE_COMM_VI_StartViChn(&m_viCfg);
    if (s32Ret != CVI_SUCCESS) {
        emxloge("StartViChn failed with %#x!\n", s32Ret);
    ...
    for (auto &item: json["rotate"]) {
        ...
        s32Ret = CVI_VI_SetChnRotation(item["pipe"].asInt(),
                                       item["chn"].asInt(),
                                       (ROTATION_E) item["rotation"].asInt());
        if (s32Ret != CVI_SUCCESS) {
            emxloge("CVI_VI_SetChnRotation failed with %#x!\n", s32Ret);
        }
    }
...
ErrCodeE MediaChipSystem::SetViVpssMode(const Json::Value &json) {
    if (json.empty()) {
        emxlogi("No setting required ViVpssMode\n");
        return ErrCodeE::Success;
    }
...
ErrCodeE MediaChipSystem::CreateVpssGroup(const Json::Value &json) {
    ...
    if (s32Ret != CVI_SUCCESS) {
        emxlogc("init vpss group failed. s32Ret: 0x%x !\n", s32Ret);
        return ErrCodeE::Failure;;
    }
    ...
    if (s32Ret != CVI_SUCCESS) {
        emxlogc("start vpss group failed. s32Ret: 0x%x !\n", s32Ret);
        return ErrCodeE::Failure;;
    }
...
ErrCodeE MediaChipSystem::InitVpss(const Json::Value &json) {
    CVI_S32 s32Ret;
    if ((VI_VPSS_MODE_E) json["ViVpssMode"][0].asInt() != VI_OFFLINE_VPSS_OFFLINE) {
    ...
    for (auto &e: json["group"]) {
        if (CreateVpssGroup(e) != ErrCodeE::Success) {
            emxloge("vpss group create failed\n");
            return ErrCodeE::Failure;
        }
    }

    for (auto &e: json["bindViVpss"]) {
        s32Ret = SAMPLE_COMM_VI_Bind_VPSS(e["pipe"].asInt(), e["chn"].asInt(), e["group"].asInt());
        if (s32Ret != CVI_SUCCESS) {
            emxloge("vi pipe %d chn %d bind vpss %d failed. s32Ret: 0x%x !\n", e["pipe"].asInt(), e["chn"].asInt(),
                   e["group"].asInt(), s32Ret);
            return ErrCodeE::Failure;
        }
    }
...
ErrCodeE MediaChipSystem::InitExpandMode(const Json::Value &json) {

    ErrCodeE e = ErrCodeE::Success;
    if (json.empty())
        return e;

    if (json["mode"].asString() == "blending") {
        Json::Value jsonMode;
        Param param("media/stitchMode");
        param.Get(jsonMode);
        e = InitStitch(jsonMode["mode"].asInt(), json["blending"]);
    }

    return e;
}

void MediaChipSystem::DeInitExpandMode(const Json::Value &json) {
    if (json.empty())
        return;
    if (json["mode"].asString() == "blending") {
        DeInitStitch();
