...
namespace Emx {
    ...
    protected:
        ...
        virtual void IspUpdate();

        virtual ErrCodeE SetAmp(PROC_AMP_E AmpMode,uint8_t value);

        virtual ErrCodeE SetExposure(OptModeE expMode, ExpGainE autoGain, ExpGainE manualGain,
                                     uint8_t compensation, int32_t expTime);

        virtual ErrCodeE SetSharpness(uint8_t sharpness);
        ...
        virtual ErrCodeE SetDeNoise(uint8_t deNoise);

        virtual ErrCodeE SetWb(OptModeE wbMode, uint8_t redGain, uint8_t blueGain);
