...
#define WAV_RIFF_CHUNK_ID       MKTAG('R','I','F','F')
#define WAV_FORMAT_CHUNK_ID     MKTAG('f','m','t',' ')
#define WAV_FACT_CHUNK_ID       MKTAG('f','a','c','t')
#define WAV_DATA_CHUNK_ID       MKTAG('d','a','t','a')
#define WAV_WAVE_ID             MKTAG('W','A','V','E')
...
void wav_write_header(WavFile *self) {
    self->riff_chunk.size =
            ...
            (self->format_chunk.header.id == WAV_FORMAT_CHUNK_ID ? (sizeof(WavChunkHeader) +
                                                                    self->format_chunk.header.size) : 0) +
            (self->fact_chunk.header.id == WAV_FACT_CHUNK_ID ? (sizeof(WavChunkHeader) + self->fact_chunk.header.size)
                                                             : 0) +
            (self->data_chunk.header.id == WAV_DATA_CHUNK_ID ? (sizeof(WavChunkHeader) + self->data_chunk.header.size)
                                                             : 0);
...
void wav_init(WavFile *self, WAV_CONST char *filename, WavU32 mode) {
    ...
    //self->format_chunk.header.size = (WavU32) ((WavUIntPtr) &self->format_chunk.body.ext_size -
...
void wav_set_format(WavFile *self, WavU16 format) {
    if (!(self->mode & WAV_OPEN_WRITE) &&
        !((self->mode & WAV_OPEN_APPEND) && self->is_a_new_file && self->data_chunk.header.size == 0)) {
        wav_err_set_literal(WAV_ERR_MODE, "This WavFile is not writable");
        return;
    }
...
void wav_set_num_channels(WavFile *self, WavU16 num_channels) {
    ...
    self->format_chunk.body.avg_bytes_per_sec =
            self->format_chunk.body.block_align * self->format_chunk.body.sample_rate;
...
void wav_set_valid_bits_per_sample(WavFile *self, WavU16 bits) {
    ...
    if ((self->format_chunk.body.format_tag == WAV_FORMAT_ALAW ||
         self->format_chunk.body.format_tag == WAV_FORMAT_MULAW) && bits != 8) {
        wav_err_set(WAV_ERR_PARAM, "Invalid ValidBitsPerSample: %u", bits);
        return;
    }
    ...
    if (self->format_chunk.body.format_tag != WAV_FORMAT_EXTENSIBLE) {
        ...
        self->format_chunk.body.bits_per_sample =
                8 * self->format_chunk.body.block_align / self->format_chunk.body.num_channels;
