...
bool OutlineOsd::IsExistChar(int ch) {
    FT_UInt code = FT_Get_Char_Index(_ftFace, ch);
    printf("Line[%d]->code[%d]\n", __LINE__, code);
    if (code == 0) {
        return false;
    }
    FT_Load_Glyph(_ftFace, code, FT_LOAD_NO_BITMAP);
    if (_ftFace->glyph->format != FT_GLYPH_FORMAT_OUTLINE) {
        printf("Glyph is empty!\n");
        return false;
    }
    if (_ftFace->glyph->outline.n_points == 0) {
        printf("Glyph is empty! n_points is null\n");
        return false;
    }
    // if (_ftFace->glyph->outline.flags & FT_OUTLINE_HIGH_PRECISION) {
    //     return true;
    // }
    // printf("Glyph is empty! no outline!\n");
    return true;
}
...
int OutlineOsd::WriteChar(int ch,int thickness)
{
    ...
    FT_UInt code = FT_Get_Char_Index(_ftFace, ch);
    printf("Line[%d]->code[%d]\n", __LINE__, code);
    err = FT_Load_Glyph(_ftFace, code, FT_LOAD_NO_BITMAP);
    if (err)
    {
