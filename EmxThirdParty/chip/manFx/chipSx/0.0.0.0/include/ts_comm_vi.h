...
typedef struct tsVI_LOW_DELAY_INFO_S {
    TS_BOOL bEnable;          /* RW; Low delay enable. */
    TS_U32 u32LineCnt;        /* RW; Range: [32, 16384]; Low delay shoreline. */
} VI_LOW_DELAY_INFO_S;

/* Information of raw data cmpresss param */
typedef struct tsVI_CMP_PARAM_S {
    TS_U8 au8CmpParam[VI_CMP_PARAM_SIZE];
} VI_CMP_PARAM_S;

typedef enum TS_VI_USERPIC_MODE_E {
...
/* Input mode */
typedef enum tsVI_INPUT_MODE_E {
...
typedef enum tsVI_YUV_DATA_SEQ_E {
...
/* Component mode */
typedef enum tsVI_COMPONENT_MODE_E {
...
typedef enum tsVI_COMBINE_MODE_E {
...
typedef enum tsVI_VSYNC_VALID_E {
...
typedef enum tsVI_VSYNC_VALID_NEG_E {
...
typedef struct tsVI_TIMING_BLANK_S {
    TS_U32 u32HsyncHfb ;    /* RW;Horizontal front blanking width */
    TS_U32 u32HsyncAct ;    /* RW;Horizontal effetive width */
    TS_U32 u32HsyncHbb ;    /* RW;Horizontal back blanking width */
    TS_U32 u32VsyncVfb ;    /* RW;Vertical front blanking height of one frame or odd-field frame picture */
    TS_U32 u32VsyncVact ;   /* RW;Vertical effetive width of one frame or odd-field frame picture */
    TS_U32 u32VsyncVbb ;    /* RW;Vertical back blanking height of one frame or odd-field frame picture */
    TS_U32 u32VsyncVbfb ;   /* RW;Even-field vertical front blanking height when input mode is interlace
                            (invalid when progressive input mode) */
    TS_U32 u32VsyncVbact ;  /* RW;Even-field vertical effetive width when input mode is interlace
                            (invalid when progressive input mode) */
    TS_U32 u32VsyncVbbb ;   /* RW;Even-field vertical back blanking height when input mode is interlace
                            (invalid when progressive input mode) */
} VI_TIMING_BLANK_S;

/* synchronization information about the BT.601 or DC timing */
typedef struct tsVI_SYNC_CFG_S {
    VI_VSYNC_E              enVsync;
...
typedef enum tsVI_BT656_FIXCODE_E {
...
typedef enum tsVI_BT656_FIELD_POLAR_E {
...
typedef struct tsVI_BT656_SYNC_CFG_S {
    VI_BT656_FIXCODE_E     enFixCode;
    VI_BT656_FIELD_POLAR_E enFieldPolar;
} VI_BT656_SYNC_CFG_S;

/* Input data type */
typedef enum tsVI_DATA_TYPE_E {
    VI_DATA_TYPE_YUV = 0,
    VI_DATA_TYPE_RGB,
...
typedef enum tsVI_REPHASE_MODE_E {
...
typedef struct tsVI_DEV_ATTR_S {
    ...
    TS_BOOL             bDataReverse;                   /* RW;Data reverse */
...
typedef struct tsVI_DEV_BIND_PIPE_S {
    TS_U32  u32Num;                                     /* RW;Range [1,VI_MAX_PHY_PIPE_NUM] */
    VI_PIPE PipeId[VI_MAX_PHY_PIPE_NUM];                /* RW;Array of pipe ID */
} VI_DEV_BIND_PIPE_S;

/* Source of 3DNR reference frame, Not support in dt56**/
typedef enum tsVI_NR_REF_SOURCE_E {
...
/* The attributes of 3DNR, Not support in dt56**/
typedef struct tsVI_NR_ATTR_S {
...
#ifdef TX536
    TS_BOOL               bNrEn;                  /* RW;Range:[0, 1];3DNR enable,Not support in dt56**/
    VI_NR_ATTR_S          stNrAttr;               /* RW;Attribute of 3DNR,Not support in dt56**/
#endif
    TS_BOOL               bSharpenEn;             /* RW;Range:[0, 1];Sharpen enable */
...
typedef enum tsVI_STITCH_ISP_CFG_MODE_E {
...
typedef struct tsVI_STITCH_GRP_ATTR_S {
    TS_BOOL                     bStitch;
    VI_STITCH_ISP_CFG_MODE_E    enMode;
    TS_U32                      u32MaxPTSGap;            /* RW;MAX PTS Gap between frame of pipe,unit:us */
    TS_U32                      u32PipeNum;              /* RW;Range [2, VI_MAX_PIPE_NUM] */
    VI_PIPE                     PipeId[VI_MAX_PIPE_NUM]; /* RW;Array of pipe ID */
} VI_STITCH_GRP_ATTR_S;

typedef enum tsVI_PIPE_REPEAT_MODE_E {
...
typedef struct {
    TS_U8   IES;              /* RW; Range:[0, 255];Format 8.0;the absolute strength of image enhancement for edge */
    TS_U8   IESS;             /* RW; Range:[0, 255];Format 8.0;the absolute strength of image enhancement for
                            texture and shadow */
    TS_U16  IEDZ;             /* RW; Range:[0, 8192];Format 14.0;the threshold of image enhancement
                            for controlling noise */
} tV59aIEy;

typedef struct {
    TS_U8   SBF     : 2;      /* RW; Range:[0, 3];Format 2.0;the band type of spatial filter, notice: SBF0, SBF1
                            range is [2, 3];SBF2,SBF3,SBF4 range is [0,3], where SBF4 is related to SBFk */
    TS_U8   STR     : 4;      /* RW; Range:[0, 13];Format 4.0;the relative strength of spatial filter refer to
                            the previous frame */
    TS_U8   STHp    : 2;      /* RW; Range:[0, 2];Format 2.0;Not recommended for debugging */
    TS_U8   SFT     : 5;      /* RW; Range:[0, 31];Format 5.0;Not recommended for debugging */
    TS_U8   kPro    : 3;      /* RW; Range:[0, 7];Format 3.0;notice: the kPro of SFy2 range is [0, 7], the kPro of
                            SFy3 range is [0, 4] */

    TS_U16  STH[3];           /* RW; Range:[0, 999];Format 10.0;the edge-preserve threshold for spatial filter */
    TS_U16  SBS[3];           /* RW; Range:[0, 9999];Format 14.0;the noise reduction strength of spatial filter
                            for the relative bright pixel */
    TS_U16  SDS[3];           /* RW; Range:[0, 9999];Format 14.0;the noise reduction strength of spatial filter
                            for the relative dark pixel */

} tV59aSFy;

typedef struct {
    TS_U16  MATH    : 10;    /* RW; Range:[0, 1023];Format 10.0;the motion detection threshold for temporal filter */
    TS_U16  MATE    : 4;     /* RW; Range:[0, 11];Format 4.0;the motion detection index of flat area
                            for temporal filter */
    TS_U16  MATW    : 2;     /* RW; Range:[0, 3];Format 2.0;the index of suppressing trailing for temporal filter */
    TS_U8   MASW    : 4;     /* RW; Range:[0, 12];Format 4.0;the index of suppressing raindrop noise
                            for temporal filter */
    TS_U8   MABW    : 3;     /* RW; Range:[0, 4];Format 3.0;the window of motion detection for temporal filter */
    TS_U8   MAXN    : 1;     /* RW; Range:[0, 1];Format 1.0;Not recommended for debugging */

} tV59aMDy;

typedef struct {
    TS_U8   TFR[4];         /* RW; Range:[0, 255];Format 8.0;the relative strength of temporal
                            filter for the static area */
    TS_U16  TDZ : 14;       /* RW; Range:[0, 999];Format 10.0;the threshold of dead-area of temporal filter */
    TS_U16  TDX : 2;        /* RW; Range:[0, 2];Format 2.0;Not recommended for debugging */
    TS_U16  TFS : 6;        /* RW; Range:[0, 63];Format 6.0;the absolute strength of temporal filter  */
} tV59aTFy;

typedef struct {
    TS_U16  SFC     : 10;   /* RW; Range:[0, 1023];Format 10.0;the strength of spatial filter for NRC0 */
    TS_U16  TFC     : 6;    /* RW; Range:[0, 63];Format 6.0;the strength of temporal filter for NRC0 */
    TS_U16  CSFS    : 14;   /* RW; Range:[0, 999];Format 10.0;the strength of spatial filter for NRC1 */
    TS_U16  CSFk    : 2;    /* RW; Range:[0, 3];Format 2.0;Not recommended for debugging */
    TS_U16  CTFS    : 4;    /* RW; Range:[0, 15];Format 4.0;the strength of temporal filter for NRC1 */
    TS_U16  CIIR    : 1;    /* RW; Range:[0, 1];Format 1.0;the mode of spatial filter for NRC1 */
    TS_U16  CTFR    : 11;   /* RW; Range:[0, 999];Format 10.0;the relative strength of temporal filter for NRC1 */
    TS_U8   MODE    : 1;    /* RW; Range: [0,  1]; The switch for new chroma denoise mode. */
    TS_U8   PRESFC  : 6;    /* RW; Range: [0,  32]; The strength for chroma pre spatial filter. */
    TS_U8  _rb1_    : 1;

} tV59aNRc;

/* 3DNR Spatial Filter: SFy0,SFy1,SFy2,SFy3; Temporal Filter:TFy0,TFy1;Chroma Noise Reduction: NRC0,NRC1,Not support in dt56**/
typedef struct {
    tV59aIEy  IEy;
    tV59aSFy  SFy[5];
    tV59aMDy  MDy[2];
    ...
    TS_U16  HdgType : 1;   /* RW; Range:[0, 1];Format 1.0;the type of complexed mixed spatial filter whether
                            is SFi or SFk */
    TS_U16  BriType : 1;   /* RW; Range:[0, 1];Format 1.0;the mode decide SFy3 whether is SFk type or SFi type */
    TS_U16  HdgMode : 2;   /* RW; Range:[0, 3];Format 2.0;the mode decide complexed mixed spatial filter band
                            for flat area */
    TS_U16  kTab2   : 1;   /* RW; Range:[0, 1];Format 1.0;the parameter decide SFy2 whether or not based on the image
                            absolute luminance */
    TS_U16  HdgWnd  : 1;   /* RW; Range:[0, 1];Format 1.0;the sampling window of complexed mixed spatial filter for
                            noise detection */
    TS_U16  kTab3   : 1;   /* RW; Range:[0, 1];Format 1.0;the parameter decide SFy3 whether or not based on the image
                            absolute luminance */
    TS_U16  HdgSFR  : 4;   /* RW; Range:[0, 13];Format 4.0;the trend of the noise reduction of complexed mixed spatial
                            filter for flat area */
    TS_U16  nOut    : 5;   /* RW; Range:[0, 27];Format 5.0;the parameter for output intermediate result of SFy3 */
    TS_U8   HdgIES;        /* RW; Range:[0, 255];Format 8.0;the strength of image enhancement for complexed
                            mixed spatial filter */
    TS_U8   nRef    : 1;   /* RW; Range:[0, 1];Format 1.0;Not recommended for debugging */

    TS_U8   IEyMode : 1;   /* RW; Range:[0, 1];Format 1.0;the image enhancement mode selection. */
    TS_U8   IEyEx[4];      /* RW; Range:[0, 255];Format 8.0;the image enhancement strength for different frequency. */

    TS_U8   SFRi[4];       /* RW; Range:[0, 255];Format 8.0;the relative strength of SFy3 when the filter type is SFi */
    TS_U8   SFRk[4];       /* RW; Range:[0, 255];Format 8.0;the relative strength of SFy3 when the filter type is SFk */
    TS_U16  SBSk2[32];     /* RW; Range:[0, 9999];Format 14.0;the noise reduction strength of SFy2 for the
                            relative bright pixel based on the image absolute luminance */
    TS_U16  SBSk3[32];     /* RW; Range:[0, 9999];Format 14.0;the noise reduction strength of SFy3 for the
                            relative bright pixel based on the image absolute luminance */
    TS_U16  SDSk2[32];     /* RW; Range:[0, 9999];Format 14.0;the noise reduction strength of SFy2 for the
                            relative dark pixel based on the image absolute luminance */
    TS_U16  SDSk3[32];     /* RW; Range:[0, 9999];Format 14.0;the noise reduction strength of SFy3 for the
                            relative dark pixel based on the image absolute luminance */
    TS_U16  BriThr[16];    /* RW; Range:[0, 1024];Format 11.0;the threshold decide SFy3 choose the SFi type filter
                            or SFk type filter in dark and bright area */

    tV59aNRc NRc;
} VI_PIPE_NRX_PARAM_V1_S;

typedef enum tsVI_NR_VERSION_E {
    VI_NR_V1 = 1,
    VI_NR_V2 = 2,
    VI_NR_V3 = 3,
    VI_NR_V4 = 4,
...
typedef struct tsNRX_PARAM_MANUAL_V1_S {
    VI_PIPE_NRX_PARAM_V1_S stNRXParamV1;
} NRX_PARAM_MANUAL_V1_S;

typedef struct tsNRX_PARAM_AUTO_V1_S {
    TS_U32                              u32ParamNum;
    TS_U32                  ATTRIBUTE   *pau32ISO;
    VI_PIPE_NRX_PARAM_V1_S  ATTRIBUTE   *pastNRXParamV1;
} NRX_PARAM_AUTO_V1_S;

typedef struct tsNRX_PARAM_V1_S {
...
typedef struct {
    TS_U8  IES0, IES1, IES2, IES3;  /* IES0~4 ; Range: [0, 255]; The gains of edge and texture enhancement.
                                    0~3 for different frequency response. */
    TS_U16 IEDZ : 10, _rb_ : 6;     /* IEDZ   ; Range: [0, 999]; The threshold to control the generated artifacts. */
} tV500_VI_IEy;

typedef struct {
    TS_U8  SPN6 : 3, SFR  : 5;      /* SPN6; Range: [0, 5];  The selection of filters to be mixed for NO.6 filter. */
                                    /* SFR ; Range: [0, 31];  The relative NR strength in the SFi and SFk filter. */
    TS_U8  SBN6 : 3, PBR6 : 5;      /* SBN6; Range: [0, 5];  The selection of filters to be mixed for NO.6 filter. */
                                    /* PBR6; Range: [0, 16];  The mix ratio between SPN6 and SBN6. */
    TS_U16 SRT0 : 5, SRT1 : 5, JMODE : 3, DeIdx : 3;    /* JMODE;      Range: [0, 4]; The selection modes
    ...
    TS_U8  DeRate, SFR6[3];                             /* DeRate;     Range: [0, 255]; The enhancement strength
    ...
    TS_U8  SFS1, SFT1, SBR1;                            /* SFS1, SFT1, SBR1; Range: [0, 255];  The NR strength
                                                         parameters for NO.1 filter. */
    TS_U8  SFS2, SFT2, SBR2;                            /* SFS2, SFT2, SBR2; Range: [0, 255];  The NR strength
                                                         parameters for NO.2 filter. */
    TS_U8  SFS4, SFT4, SBR4;                            /* SFS4, SFT4, SBR4; Range: [0, 255];  The NR strength
                                                         parameters for NO.3 and NO.4 filters. */

    TS_U16 STH1 : 9, SFN1 : 3, NRyEn : 1, SFN0  : 3;    /* STH1~3; Range: [0, 511]; The thresholds for protection
                                                         of edges from blurring */
                                                        /* NRyEn ; Range: [0, 1]; The NR switches */
    TS_U16 STH2 : 9, SFN2 : 3, BWSF4 : 1, kMode : 3;    /* SFN0~3; Range: [0, 6]; Filter selection for different
                                                         image areas based on STH1~3. */
                                                        /* BWSF4 ; Range: [0, 1]; The NR window size for the
                                                         NO.3 and NO.4 filters.  */
    TS_U16 STH3 : 9, SFN3 : 3, TriTh : 1, _rb0_ : 3;    /* KMode ; Range: [0, 3]; The denoise mode based
                                                         on image brightness. */
...
typedef struct tsNRX_PARAM_MANUAL_V2_S {
    VI_PIPE_NRX_PARAM_V2_S stNRXParamV2;
} NRX_PARAM_MANUAL_V2_S;

typedef struct tsNRX_PARAM_AUTO_V2_S {
    TS_U32                              u32ParamNum;
    TS_U32                  ATTRIBUTE   *pau32ISO;
    VI_PIPE_NRX_PARAM_V2_S  ATTRIBUTE   *pastNRXParamV2;
} NRX_PARAM_AUTO_V2_S;

typedef struct tsNRX_PARAM_V2_S {
    OPERATION_MODE_E        enOptMode;           /* RW;Adaptive NR */
...
/**Not support in dt56**/
typedef struct tsVI_PIPE_NRX_PARAM_S {
...
typedef enum tsVI_VS_SIGNAL_MODE_E {
...
typedef struct tsVI_VS_SIGNAL_ATTR_S {
    VI_VS_SIGNAL_MODE_E enMode;             /* RW;output one time, output frequently */
    TS_U32              u32StartTime;       /* RW;output start time,unit: sensor pix clk. */
    TS_U32              u32Duration;        /* RW;output high duration, unit: sensor pix clk. */
    TS_U32              u32CapFrmIndex;     /* RW;VS signal will be output after trigger by which vframe,
                                            default is 0. */
    TS_U32              u32Interval;        /* RW;output frequently interval, unit: frame */
} VI_VS_SIGNAL_ATTR_S;

typedef struct tsBNR_DUMP_ATTR_S {
    TS_BOOL bEnable;
    TS_U32 u32Depth;
} BNR_DUMP_ATTR_S;

typedef enum tsVI_EXT_CHN_SOURCE_E {
...
typedef struct tsVI_EXT_CHN_ATTR_S {
...
typedef enum tsVI_CROP_COORDINATE_E {
...
typedef struct tsVI_CROP_INFO_S {
    TS_BOOL                 bEnable;            /* RW;CROP enable */
...
typedef struct tsVI_LDC_ATTR_S {
    TS_BOOL bEnable; /* RW;Range [0,1];Whether LDC is enbale */
    LDC_ATTR_S stAttr;
} VI_LDC_ATTR_S;

/* The attributes of LDCV2 */
typedef struct tsVI_LDCV2_ATTR_S {
    TS_BOOL bEnable; /* RW;Whether LDC is enbale */
    LDCV2_ATTR_S stAttr;
} VI_LDCV2_ATTR_S;

/* The attributes of LDCV3 */
typedef struct tsVI_LDCV3_ATTR_S {
    TS_BOOL bEnable; /* RW;Whether LDC is enbale */
    LDCV3_ATTR_S stAttr;
} VI_LDCV3_ATTR_S;

typedef struct tsVI_ROTATION_EX_ATTR_S {
    TS_BOOL       bEnable;                 /* RW;Range [0,1];Whether ROTATE_EX_S is enbale */
...
typedef struct tsVI_PMF_ATTR_S {
    TS_BOOL bEnable;                            /* RW;Whether PMF is enable */
    SIZE_S  stDestSize;                         /* RW;Target size */
    TS_S64  as64PMFCoef[VI_PMFCOEF_NUM];        /* RW; Array of PMF coefficients */
} VI_PMF_ATTR_S;
...
typedef struct tsVI_DUMP_ATTR_S {
    TS_BOOL         bEnable;              /* RW;Whether dump is enable */
    TS_U32          u32Depth;             /* RW;Range [0,8];Depth */
...
typedef struct TS_VI_RAW_INFO_S {
    VIDEO_FRAME_INFO_S      stVideoFrame;
    ISP_CONFIG_INFO_S       stIspInfo;
} VI_RAW_INFO_S;

/* module params */
typedef struct tsVI_MOD_PARAM_S {
    TS_S32      s32DetectErrFrame;
    TS_U32      u32DropErrFrame;
    VB_SOURCE_E enViVbSource;
} VI_MOD_PARAM_S;

typedef struct tsVI_DEV_TIMING_ATTR_S {
    TS_BOOL bEnable;               /* RW;Range:[0,1];Whether enable VI generate timing */
    TS_S32  s32FrmRate;            /* RW;Range:(0,0xffffff];;Generate timing Frame rate */
} VI_DEV_TIMING_ATTR_S;

typedef struct tsVI_EARLY_INTERRUPT_S {
    TS_BOOL bEnable;
    TS_U32 u32LineCnt;
