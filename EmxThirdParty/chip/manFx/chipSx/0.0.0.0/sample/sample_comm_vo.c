...
TS_S32 SAMPLE_COMM_VO_GetDefConfig(SAMPLE_VO_CONFIG_S *pstVoConfig)
{
    ...
    TS_MPI_SYS_GetChipId(&u32ChipId);
...
TS_S32 SAMPLE_COMM_VO_StartVO(SAMPLE_VO_CONFIG_S *pstVoConfig)
{
...
#ifdef MPP_USE_PCSIMULATOR//x86
	stVoPubAttr.enIntfSync = VO_OUTPUT_768P60;
	stVoPubAttr.format = VO_FORMAT_AR24;
#else//arm
	stVoPubAttr.enIntfSync = VO_OUTPUT_720P60;
	stVoPubAttr.format = VO_FORMAT_NV12;
#endif

#else
	SAMPLE_COMM_VO_GetPubAttr(SENSOR0_TYPE, &stVoPubAttr);
#endif
    ...
    ********************************/
    ...
    {
        s32Ret = TS_MPI_VO_SetVideoLayerPartitionMode(VoLayer, enVoPartMode);
        if (TS_SUCCESS != s32Ret)
        {
            SAMPLE_PRT("TS_MPI_VO_SetVideoLayerPartitionMode failed!\n");
            SAMPLE_COMM_VO_StopDev(VoDev);
            return s32Ret;
        }
    }
