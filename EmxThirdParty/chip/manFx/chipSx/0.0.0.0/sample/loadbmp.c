...
TS_U16 OSD_MAKECOLOR_U16(TS_U8 r, TS_U8 g, TS_U8 b, OSD_COMP_INFO compinfo)
{
	TS_U8 r1, g1, b1;
	TS_U16 pixel = 0;
	TS_U32 tmp = 15;
...
TS_S32 GetBmpInfo(const char *filename,   OSD_BITMAPFILEHEADER *pBmpFileHeader, OSD_BITMAPINFO *pBmpInfo)
{
	FILE *pFile;
...
int LoadBMP(const char *filename, OSD_LOGO_T *pVideoLogo)
{
	...
	pVideoLogo->width = (TS_U16)bmpInfo.bmiHeader.biWidth;
	pVideoLogo->height = (TS_U16)((bmpInfo.bmiHeader.biHeight > 0) ? bmpInfo.bmiHeader.biHeight :
...
#endif
	/* RGB8888 or RGB1555 */
	pOrigBMPBuf = (TS_U8 *)malloc(h * stride);
	if (NULL == pOrigBMPBuf) {
	...
	for (i = 0; i < h; i++) {
		for (j = 0; j < w; j++) {
			memcpy(pRGBBuf + i * pVideoLogo->stride + j * dstBpp, pOrigBMPBuf + ((h - 1) - i)*stride +
													j * Bpp, Bpp);
		...
		for (j = 0; j < w; j++) {
			if (Bpp == 3) {
				...
				case OSD_COLOR_FMT_RGB4444:
					...
					pDst = (TS_U16 *)(pRGBBuf + i * pVideoLogo->stride + j * 2);
					r = *(pStart);
					g = *(pStart + 1);
					b = *(pStart + 2);
				...
				memcpy(pRGBBuf + i * pVideoLogo->stride + j * 4, pOrigBMPBuf + ((h - 1) - i)*stride +
								j * Bpp, Bpp);
				...
				*(pRGBBuf + i * pVideoLogo->stride + j * 4 + 3) = 0xff; /*alpha*/
				...
				memcpy(pRGBBuf + i * pVideoLogo->stride + j * Bpp, pOrigBMPBuf + ((h - 1) - i)*stride +
								j * Bpp, Bpp);
...
int LoadBMPCanvas(const char *filename, OSD_LOGO_T *pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
	...
	w = (TS_U16)bmpInfo.bmiHeader.biWidth;
	h = (TS_U16)((bmpInfo.bmiHeader.biHeight > 0) ? bmpInfo.bmiHeader.biHeight : (-bmpInfo.bmiHeader.biHeight));

	stride = w * Bpp;
    printf("##OSD_LoadBMP: stride=%d, Bpp=%d, w=%d, pVideoLogo->stride=%d\n", stride, Bpp, w, pVideoLogo->stride);
