...
TS_S32 main(TS_S32 argc, TS_CHAR **argv)
{
    ...
    for (TS_S32 n = 0; n < num; ++n) {
        ...
        TS_MPI_TRP_RNE_Info("outputBlobs2 crc16 = %04x\n",
                            RNE_GetBlobsCRC16CCITT(outputBlobs2));
        TS_MPI_TRP_RNE_Info("outputBlobs2 crc32 = %08x\n",
                            RNE_GetBlobsCRC32(outputBlobs2));
        /* 调用TS_MPI_TRP_RNE_GetHideBlobs 打印hide blobs
         */
        RNE_BLOBS_S *hideBlobs = TS_MPI_TRP_RNE_GetHideBlobs(net[n]);
        TS_MPI_TRP_RNE_Info("outputBlobs2 crc16 = %04x\n",
                            RNE_GetBlobsCRC16CCITT(hideBlobs));
        TS_MPI_TRP_RNE_Info("outputBlobs2 crc32 = %08x\n",
                            RNE_GetBlobsCRC32(hideBlobs));
