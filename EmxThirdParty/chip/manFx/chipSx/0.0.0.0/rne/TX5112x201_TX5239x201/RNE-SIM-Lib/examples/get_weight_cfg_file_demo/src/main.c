...
static TS_S32 TS_MPI_TRP_RNE_InitNetsGraphAndParams(RNE_NET_S **net, TS_U8 **paramStride, TS_S32 *paramSize,
                                                    TS_S32 nNet)
{
    /* 3 打开RNE设备
     */
...
TS_S32 TS_MPI_TRP_RNE_BinFileFilter(const struct dirent *fd)
{
    ...
    if (!strcmp(BIN_SUFFIX, fd->d_name + len - suffix_len) && fd->d_type != DT_DIR) {
        return 1;
    }
    return 0;
}
...
TS_S32 TS_MPI_TRP_RNE_CfgFileFilter(const struct dirent *fd)
{
    ...
    if (!strcmp(CFG_SUFFIX, fd->d_name + len - suffix_len) && fd->d_type != DT_DIR) {
        return 1;
    }
    return 0;
}
...
TS_S32 TS_MPI_TRP_RNE_WeightFileFilter(const struct dirent *fd)
{
    ...
    if (!strcmp(WEIGHT_SUFFIX, fd->d_name + len - suffix_len) && fd->d_type != DT_DIR) {
        return 1;
    }
    return 0;
}

TS_S32 TS_MPI_TRP_RNE_GetFileList(const TS_CHAR *type, const TS_CHAR *path, struct dirent ***f_l)
{
    ...
    if (!strcmp(type, CFG_SUFFIX)) {
        ...
        *f_l = nameList;
        ...
        num = scandir(path, &nameList, TS_MPI_TRP_RNE_WeightFileFilter, versionsort);
        *f_l = nameList;
...
TS_VOID TS_MPI_TRP_RNE_ReadBinData(TS_CHAR *fileName, uint8_t *buf, TS_S32 size)
{
    FILE *file = NULL;
    TS_S32 rc = 0;
...
TS_S32 main(TS_S32 argc, TS_CHAR **argv)
{
    ...
    TS_MPI_TRP_RNE_InitResourceByType(RNE_TYPE_NAME_TX5112CV201);
    /* 1 设置log等级
     */
    ...
    TS_S32 size = strlen(srcDir) + 1 + strlen((TS_CHAR *)cfg_nameList[0]->d_name) + 1;
    ...
    TS_S32 weight_size = strlen(srcDir) + 1 + strlen((TS_CHAR *)weight_nameList[0]->d_name) + 1;
    ...
    TS_MPI_TRP_RNE_ReadBinData(weight_fileName, (uint8_t *)weightData, weightFileSize);
    ...
    if (0 != TS_MPI_TRP_RNE_InitNetsGraphAndParams(net, paramStride, paramSize, num)) {
    ...
    for (TS_S32 n = 0; n < inputNum; ++n) {
        size = strlen(srcDir) + 1 + strlen((TS_CHAR *)input_nameList[n]->d_name) + 1;
        ...
        TS_MPI_TRP_RNE_ReadBinData(input_fileName, (uint8_t *)inputData, inputFileSize);
