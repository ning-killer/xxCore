...
TS_S32 main(TS_S32 argc, TS_CHAR **argv)
{
    ...
    TS_MPI_TRP_RNE_InitTimeState(RNE_LAYER_TYPE_MAX_LAYER_TYPE - RNE_LAYER_TYPE_START_LAYER, &timeState);
    ...
    for (TS_S32 n = 0; n < num; ++n) {
        if (0 != TS_MPI_TRP_RNE_NetBindTimeState(net[n], &timeState)) {
            TS_MPI_TRP_RNE_Error("net bind time state error!\n");
            goto releaseResult;
        }
        ...
        if (0 != TS_MPI_TRP_RNE_FillInputBlobs(net[n], inputBlobs, 0, model_input)) {
