...
{
    if ((blob->fCoeff != NULL && (((*blob->fCoeff - 1) < (-EPSILON)) || ((*blob->fCoeff - 1) > EPSILON))) ||
        (*blob->s32pQ == -1)) {
        return true;
    } else {
        return false;
...
TS_S32 TS_MPI_TRP_RNE_FillInputBlobs(RNE_NET_S *net, RNE_BLOBS_S *blobs, TS_S32 coreIdx, TS_VOID *input)
{
    ...
    for (uint32_t idx_blob = 0; idx_blob < blobs->u32NBlob; ++idx_blob) {
        ...
        TS_S32 c_stride = TS_MPI_TRP_RNE_CStride(blob->s32C, blob->s32BitNum, blob->bIsJoined);
        ...
        TS_S64 data_min = ((TS_S64)(MIN_BIT(bits - 1) & MASK_BITS(blob->s32BitNum)) << sign_bit) >> sign_bit;
        ...
        if (in_type == RNE_NET_INPUT_TYPE_FLOAT32_HWC && !TS_MPI_TRP_RNE_InputBlobIsFloat(blob)) {
            in_type = RNE_NET_INPUT_TYPE_INT8_HWC;
        } else if (in_type == RNE_NET_INPUT_TYPE_FLOAT32_CHW && !TS_MPI_TRP_RNE_InputBlobIsFloat(blob)) {
                    ...
                    for (TS_S32 bat = 0; bat < n; bat++) {
                        for (TS_S32 i = 0; i < h * w; ++i) {
                            for (TS_S32 k = 0; k < c; ++k) {
                                ...
                                if (blob->s32BitNum == 8) {
                                    tmp /= (1 << q_scale);
                                    data = ((((TS_S64)(tmp * (1 << q_scale)) | (1LL << 62)) >> 62) * 0.5f + tmp);
                                } else {
                        ...
                        dst = (TS_U8 *)(((TS_SIZE_T)dst + BLOB_ALIGN_BYTES - 1) & (~((TS_SIZE_T)BLOB_ALIGN_BYTES - 1)));
                    }
                }
