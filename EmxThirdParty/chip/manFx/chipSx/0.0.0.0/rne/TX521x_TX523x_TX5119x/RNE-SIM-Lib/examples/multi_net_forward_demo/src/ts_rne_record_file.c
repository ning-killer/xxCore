...
TS_VOID TS_MPI_TRP_RNE_ReplaceCToC(TS_CHAR *str, TS_CHAR frontC,
                                   TS_CHAR afterC) {
  TS_CHAR *ptr = str;
  while (*ptr != '\0') {
    if (*ptr == frontC)
...
TS_S32 TS_MPI_TRP_RNE_RecordHwcPFile(TS_S32 frameIndex, TS_S32 frameCount,
                                     ...
                                     TS_CHAR *outputPath, TS_CHAR *debugName,
                                     TS_S32 outType) {
      ...
      for (TS_S32 i = 0; i < h; i++) {
        for (TS_S32 j = 0; j < w; j++) {
          ...
          for (TS_S32 k = 0; k < c; k++) {
            if (q_rst >= 0) {
              TS_S32 *d =
                  (TS_S32 *)(out + (((j + i * w) * cStride + k) * uSize));
...
TS_S32 TS_MPI_TRP_RNE_RecordFile(TS_S32 frameIndex, TS_S32 frameCount,
  ...
  if (debug == NULL)
    return -1;
#if defined(RESULT_HWC_P) || defined(RESULT_HWC) || defined(RESULT_CHW_P) ||   \
    defined(RESULT_CHW) || defined(RESULT_ALL)
