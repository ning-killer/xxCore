                                                    ...
                                                    TS_S32 nNet)
{
    ...
    for (TS_S32 i = 0; i < nNet; ++i) {
...
#endif
        {
            ...
            TS_MPI_TRP_RNE_RegisterGpUserData(net[i], 4, "This is a test for UserData!");
...
static TS_VOID TS_MPI_TRP_RNE_PrintGpLayersInfoList(RNE_NET_S *net)
{
    ...
    for (TS_S32 i = 0; i < layersInfo->u32LayersNum; ++i) {
        TS_MPI_TRP_RNE_Info("layer id: %d layer name: %s layerType:%d typeName:%s\n",
                            layersInfo->stpLayerInfo[i].s32LayerId, layersInfo->stpLayerInfo[i].cpLayerName,
                            layersInfo->stpLayerInfo[i].s32LayerType, layersInfo->stpLayerInfo[i].cpTypeName);
    }
}
