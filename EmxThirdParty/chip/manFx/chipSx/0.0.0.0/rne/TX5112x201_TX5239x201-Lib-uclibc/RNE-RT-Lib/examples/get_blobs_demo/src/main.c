...
#ifndef TS_MPI_TRP_RNE_MASK_BITS
...
TS_S32 main(TS_S32 argc, TS_CHAR **argv)
{
    ...
    for (TS_S32 n = 0; n < num; ++n) {
        ...
        /* 输出结果，此结果与TS_MPI_TRP_RNE_GetResultBlobs结果应该一致
         */
        /* 调用TS_MPI_TRP_RNE_GetResultBlobs
         */
        RNE_BLOBS_S *outputBlobs2 = TS_MPI_TRP_RNE_GetResultBlobs(net[n]);
        TS_MPI_TRP_RNE_Info("outputBlobs2 crc = %04x\n", TS_MPI_TRP_RNE_GetBlobsCRC16CCITT(outputBlobs2));
        /* 调用TS_MPI_TRP_RNE_GetHideBlobs 打印hide blobs
         */
        RNE_BLOBS_S *hideBlobs = TS_MPI_TRP_RNE_GetHideBlobs(net[n]);
        TS_MPI_TRP_RNE_Info("hideBlobs crc = %04x\n", TS_MPI_TRP_RNE_GetBlobsCRC16CCITT(hideBlobs));
