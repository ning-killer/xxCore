...
libs_outpath_demo=./build_linux_a7/Release/${demo_name}.elf
#${libs_outpath_demo}
#result=$?
 
if [ -f $libs_outpath_demo ]; then
    echo_sucess "==========complie demo: "${demo_name}" SUCCESS!=========="
else
    echo_error "==========complie demo: "${demo_name}" FAILED!=========="
    exit 1
fi
