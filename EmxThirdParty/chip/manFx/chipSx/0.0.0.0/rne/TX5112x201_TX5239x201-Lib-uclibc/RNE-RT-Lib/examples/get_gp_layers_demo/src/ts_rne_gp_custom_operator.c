 ...
 *
 */
void TS_MPI_TRP_RNE_GpLayerCustormOperator(RNE_BLOBS_S *input, RNE_BLOBS_S *output, RNE_BIN_DATA_S *extraData,
                                           void *userData)
{
    // userData test
    if (NULL != userData) {
    ...
    TS_MPI_TRP_RNE_Info(
        ...
        input->u32NBlob, input->stpBlob->s32H, input->stpBlob->s32W, input->stpBlob->s32C, input->stpBlob->bIsJoined,
        input->stpBlob->bIsUnsigned, input->stpBlob->s32BitNum, *(input->stpBlob->s32pQ), *(input->stpBlob->fCoeff));
    ...
    TS_MPI_TRP_RNE_Info(
        ...
        output->stpBlob->bIsJoined, output->stpBlob->bIsUnsigned, output->stpBlob->s32BitNum, *(output->stpBlob->s32pQ),
        *(output->stpBlob->fCoeff));
    ...
    for (TS_S32 idx = 0; idx < input->u32NBlob; idx++) {
        size +=
            input->stpBlob->s32H * input->stpBlob->s32W * input->stpBlob->s32C * (input->stpBlob->s32BitNum / CHAR_BIT);
    }
