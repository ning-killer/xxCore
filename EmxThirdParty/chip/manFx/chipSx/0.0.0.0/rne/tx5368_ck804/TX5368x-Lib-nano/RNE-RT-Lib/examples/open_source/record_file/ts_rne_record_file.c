...
#define THIS_IS_SAME_CODE memset(debugLayerNamef, 0, sizeof(debugLayerNamef));\
...
#define THIS_IS_SAME_CODE_PRINT  if (frameIndex == (frameCount - 1)) {\
            ...
            for (TS_S32 i = 0; i < h; i++) {
                for (TS_S32 j = 0; j < w; j++) {
                    ...
                    for (TS_S32 k = 0; k < c; k++) {
                        if (q_rst >= 0) {
                            TS_S32 *d = (TS_S32 *)(out + (((j + i * w) * cStride + k) *
                                                    uSize));
...
TS_S32 TS_MPI_TRP_RNE_RecordFile(TS_S32 frameIndex, TS_S32 frameCount,
                RNE_BLOBS_S *debug, TS_CHAR *debugLayerName,
    ...
    TS_MPI_TRP_RNE_RecordHwcPFile(frameIndex,frameCount, debug,
            debugLayerName, outputPath, debugName, outType);
    TS_MPI_TRP_RNE_RecordHwcFile(frameIndex,frameCount, debug,
            debugLayerName, outputPath, debugName, outType);
    TS_MPI_TRP_RNE_RecordChwPFile(frameIndex,frameCount, debug,
            debugLayerName, outputPath, debugName, outType);
    TS_MPI_TRP_RNE_RecordChwFile(frameIndex,frameCount, debug,
            debugLayerName, outputPath, debugName, outType);
