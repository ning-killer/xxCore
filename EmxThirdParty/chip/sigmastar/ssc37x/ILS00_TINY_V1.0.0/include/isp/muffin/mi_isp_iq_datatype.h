...
#define NRLUMA_KNL_BY_DIFF_NUM       32
#define NRLUMA_KNL_GAIN_NUM          12
#define NRLUMA_KNL_SHIFT_NUM         11
...
#define SHARPNESS_DER_FREQ_NUM       2
#define SHARPNESS_EX_LUT_X_NUM       6
#define SHARPNESS_EX_LUT_Y_NUM       7
#define SHARPNESS_EX_BY_Y_NUM        9
#define SHARPNESS_EX_BY_MOT_NUM      16
#define SHARPNESS_EX_BY_HUE_NUM      24
#define SHARPNESS_EX_BY_SAT_NUM      2
#define CROSSTALK_BY_Y_NUM           15
#define WDR_BY_Y_NUM                 33
#define WDR_LCE_BY_Y_NUM             9
...
typedef struct MI_ISP_IQ_Nr3dParam_s
{
    ...
    MI_U8  u8YSfBlendLut[NR3D_BY_MOT_NUM];        // 0~16
    ...
    MI_U8  bESEn;                                 // 0~1
    MI_U8  u8ESYGain[NR3D_ES_YGAIN_Y_NUM];        // 0~255
    MI_U8  u8ESYShift[NR3D_ES_YGAIN_X_NUM];       // 0~7
    MI_U8  u8ESStdRemapX[NR3D_ES_STDREMAP_X_NUM]; // 0~255
    MI_U8  u8ESStdRemapY[NR3D_ES_STDREMAP_Y_NUM]; // 0~255
    MI_U8  u8ESStd[NR3D_ES_STDGAIN_X_NUM];        // 0~255
    MI_U8  u8ESStdGain[NR3D_ES_STDGAIN_Y_NUM];    // 0~16
    MI_U8  u8ESMotGain[NR3D_BY_MOT_NUM];          // 0~16
...
typedef struct MI_ISP_IQ_Nr3dExParam_s
{
    ...
    MI_ISP_IQ_Bool_e bPREn;                                      // 0~1
    MI_U16           u16PRDiffTh;                                // 0~4095
    MI_U8            u8PRRatioConf[NR3D_EX_PR_RATIO_CONF_NUM];   // 0~16
    MI_U8            u8PRContrastTh[NR3D_EX_PR_CONTRAST_TH_NUM]; // 0~15
    MI_U8            u8PRMotTh[NR3D_EX_PR_MOT_TH_NUM];           // 0~255
...
typedef struct MI_ISP_IQ_NrDespikeParam_s
{
    ...
    MI_U8  u8StrengthByY[NRDESPIKE_BY_Y_NUM];    // 0 ~ 127
...
typedef struct MI_ISP_IQ_NrLumaParam_s
{
    ...
    MI_U8 u8PreStrength;                             // 0~63
    MI_U8 u8FilterSel;                               // 0~3
    MI_U8 u8KernelStr;                               // 0~7
    MI_U8 u8KernelStrByDiff[NRLUMA_KNL_BY_DIFF_NUM]; // 0~31
    MI_U8 u8KernelGain[NRLUMA_KNL_GAIN_NUM];         // 0~255
    MI_U8 u8KernelShift[NRLUMA_KNL_SHIFT_NUM];       // 0~7
    MI_U8 u8LumaStaticGain;                          // 0~64
...
typedef struct MI_ISP_IQ_NrLumaType_s
{
    ...
    MI_ISP_IQ_NrLumaManualAttr_t stManual;
} MI_ISP_IQ_NrLumaType_t;

typedef struct MI_ISP_IQ_NrLumaAdvParam_s
{
    MI_U8  u8DbgMode;                                   // 0~3
    MI_U16 u16EdgeTh[NRLUMA_ADV_BY_Y_NUM];              // 0~8191
    MI_U8  u8SmoDeltaSft;                               // 0~9
    MI_U16 u16SmoDeltaTh[NRLUMA_ADV_BY_Y_NUM];          // 0~1023
    MI_U8  u8SmoDeltaLut[NRLUMA_ADV_SMO_DELTA_LUT_NUM]; // 0~63
    MI_U8  u8SmoByMot[NRLUMA_ADV_BY_MOT_NUM];           // 0~64
    MI_U16 u16Strength[NRLUMA_ADV_LEVEL_NUM];           // 0~256
    MI_U8  u8StrengthByHue[NRLUMA_ADV_BY_HUE_NUM];      // 0~64
    MI_U8  u8StrengthBySat[NRLUMA_ADV_BY_SAT_NUM];      // 0~127
...
typedef struct MI_ISP_IQ_SharpnessParam_s
{
    ...
    MI_U8  u8DetailGainByMotUD[SHARPNESS_BY_MOT_NUM]; // 0 ~ 255
    MI_U16 u16RatioByStateH[SHARPNESS_BY_STATE_NUM];  // 0 ~ 256
    MI_U16 u16RatioByStateM[SHARPNESS_BY_STATE_NUM];  // 0 ~ 256
    MI_U16 u16RatioByStateML[SHARPNESS_BY_STATE_NUM]; // 0 ~ 256
    MI_U16 u16RatioByState[SHARPNESS_FREQ_NUM];       // 0 ~ 256
    MI_U16 u16BlendByMotH[SHARPNESS_BY_MOT_NUM];      // 0 ~ 256
    MI_U16 u16BlendByMotM[SHARPNESS_BY_MOT_NUM];      // 0 ~ 256
    MI_U8  u8NRStrByMotH[SHARPNESS_BY_MOT_NUM];       // 0 ~ 16
    MI_U8  u8NRStrByMotM[SHARPNESS_BY_MOT_NUM];       // 0 ~ 16
    MI_U8  u8StdAdjByMot[SHARPNESS_BY_MOT_NUM];       // 0 ~ 63
    MI_U8  u8StdAdjByY[SHARPNESS_BY_Y_NUM];           // 0 ~ 255
    MI_U8  u8GainByStdSftH[SHARPNESS_LUT_X_NUM];      // 0 ~ 15
    MI_U8  u8GainByStdH[SHARPNESS_LUT_Y_NUM];         // 0 ~ 255
    MI_U8  u8GainByStdSftM[SHARPNESS_LUT_X_NUM];      // 0 ~ 15
    MI_U8  u8GainByStdM[SHARPNESS_LUT_Y_NUM];         // 0 ~ 255
    MI_U8  u8EdgeKillLutSftH[SHARPNESS_LUT_X_NUM];    // 0 ~ 15
    MI_U8  u8EdgeKillLutSftM[SHARPNESS_LUT_X_NUM];    // 0 ~ 15
    MI_U16 u16EdgeKillLutH[SHARPNESS_LUT_Y_NUM];      // 0 ~ 1023
    MI_U16 u16EdgeKillLutM[SHARPNESS_LUT_Y_NUM];      // 0 ~ 1023
    MI_U8  u8DerEn[SHARPNESS_DER_FREQ_NUM];           // 0 ~ 1
    MI_U8  u8DerGain[SHARPNESS_DER_FREQ_NUM];         // 0 ~ 255
    MI_U8  u8DerGainByMotH[SHARPNESS_BY_MOT_NUM];     // 0 ~ 255
    MI_U8  u8DerGainByMotM[SHARPNESS_BY_MOT_NUM];     // 0 ~ 255
...
typedef struct MI_ISP_IQ_SharpnessExParam_s
{
    ...
    MI_U8            u8GainByStdSft[SHARPNESS_EX_LUT_X_NUM];   // 0 ~ 15
    MI_U8            u8GainByStd[SHARPNESS_EX_LUT_Y_NUM];      // 0 ~ 255
    MI_U8            u8CorByY[SHARPNESS_EX_BY_Y_NUM];          // 0 ~ 255
    MI_U8            u8CorByMot[SHARPNESS_EX_BY_MOT_NUM];      // 0 ~ 255
    MI_U8            u8OverShootGain;                          // 0 ~ 255
    MI_U8            u8UnderShootGain;                         // 0 ~ 255
    MI_U8            u8SclByY[SHARPNESS_EX_BY_Y_NUM];          // 0 ~ 255
    MI_U16           u16EdgeGain;                              // 0 ~ 1023
    MI_U8            u8GainByMot[SHARPNESS_EX_BY_MOT_NUM];     // 0 ~ 255
    MI_U8            u8StrengthByHue[SHARPNESS_EX_BY_HUE_NUM]; // 0 ~ 255
    MI_U8            u8StrengthBySat[SHARPNESS_EX_BY_SAT_NUM]; // 0 ~ 127
...
{
    MI_ISP_IQ_Bool_e bAutoDetailEnhance;         // 0 ~ 1
...
typedef struct MI_ISP_IQ_WdrLceParam_s
{
    ...
    MI_U16 u16ToneMapTrend;                        // 0 ~ 4095
    MI_U8  u8Coarse_BoxNum;                        // 0 ~ 3
    MI_U8  u8Coarse_FltCoef;                       // 0 ~ 4
    MI_U16 u16Coarse_FltSclByY[WDR_LCE_BY_Y_NUM];  // 0 ~ 256
    MI_U16 u16CoarseP_EnhRange;                    // 0 ~ 2047
    MI_U16 u16CoarseP_EnhStr;                      // 0 ~ 1023
    MI_U16 u16CoarseN_EnhRange;                    // 0 ~ 2047
    MI_U16 u16CoarseN_EnhStr;                      // 0 ~ 1023
    MI_U8  u8Fine_BoxNum;                          // 0 ~ 3
    MI_U8  u8Fine_FltCoef;                         // 0 ~ 4
    MI_U16 u16Fine_FltSclByY[WDR_LCE_BY_Y_NUM];    // 0 ~ 256
    MI_U16 u16FineP_EnhRange;                      // 0 ~ 2047
    MI_U16 u16FineP_EnhStr;                        // 0 ~ 1023
    MI_U16 u16FineN_EnhRange;                      // 0 ~ 2047
    MI_U16 u16FineN_EnhStr;                        // 0 ~ 1023
} MI_ISP_IQ_WdrLceParam_t;

typedef struct MI_ISP_IQ_WdrLceManualAttr_s
{
    MI_ISP_IQ_WdrLceParam_t stParaAPI;
} MI_ISP_IQ_WdrLceManualAttr_t;

typedef struct MI_ISP_IQ_WdrLceAutoAttr_s
{
    MI_ISP_IQ_WdrLceParam_t stParaAPI[MI_ISP_AUTO_NUM];
} MI_ISP_IQ_WdrLceAutoAttr_t;

typedef struct MI_ISP_IQ_WdrLceType_s
{
    MI_ISP_IQ_Bool_e             bEnable;  // 0 ~ 1
    MI_ISP_IQ_OpType_e           enOpType; // M_AUTO ~ (M_MODMAX-1)
    MI_ISP_IQ_WdrLceAutoAttr_t   stAuto;
    MI_ISP_IQ_WdrLceManualAttr_t stManual;
...
typedef struct MI_ISP_IQ_WdrNrParam_s
{
    ...
    MI_U8  u8Nr_Sft[WDR_NR_LUT_X_NUM];     // 1~15
    MI_U16 u16NrR_Thd[WDR_NR_LUT_Y_NUM];   // 0~4095
    MI_U16 u16NrR_Gain[WDR_NR_LUT_Y_NUM];  // 0~4095
    MI_U16 u16NrG_Thd[WDR_NR_LUT_Y_NUM];   // 0~4095
    MI_U16 u16NrG_Gain[WDR_NR_LUT_Y_NUM];  // 0~4095
    MI_U16 u16NrB_Thd[WDR_NR_LUT_Y_NUM];   // 0~4095
    MI_U16 u16NrB_Gain[WDR_NR_LUT_Y_NUM];  // 0~4095
    MI_U8  u8VND_Sft[WDR_VND_LUT_X_NUM];   // 1~11
    MI_U16 u16VND_Thd[WDR_VND_LUT_Y_NUM];  // 0~4095
    MI_U16 u16VND_Gain[WDR_VND_LUT_Y_NUM]; // 0~4095
...
typedef struct MI_ISP_IQ_HsvParam_s
{
    ...
    MI_S16 s16HueLut_ByIso[HSV_HUE_NUM];      //-64 ~  64
    MI_U16 u16SatLut_ByIso[HSV_HUE_NUM];      //  0 ~ 255 (64->1X)
...
typedef struct MI_ISP_IQ_HdrParam_s
{
    ...
    MI_U16           u16NoiseLevel[HDR_BY_Y_NUM];           // 0 ~ 1023
    MI_U16           u16MotionAdjLut[HDR_MOT_LUT_Y_NUM];    // 0 ~ 256
    MI_U16           u16MoLuBlendLutY[HDR_BLEND_LUT_Y_NUM]; // 0 ~ 256
    MI_U8            u8MoLuBlendLutX[HDR_BLEND_LUT_X_NUM];  // 0 ~ 7
...
typedef struct MI_ISP_IQ_HdrExParam_s
{
    ...
    MI_U8            u8MotionAdjSft[HDR_EX_MOT_LUT_X_NUM];
...
{
    MI_ISP_IQ_CsaMode_e CSAmode;
    MI_U32              u32GainLuma;
    MI_U32              u32GainHue;
    MI_U32              u32GainSat;
    MI_U32              u32LumaLut[CSA_LUT_NUM]; // work under Manual Mode
    MI_U32              u32HueLut[CSA_LUT_NUM];  // work under Manual Mode
    MI_U32              u32SatLut[CSA_LUT_NUM];  // work under Manual Mode
} MI_ISP_IQ_CsaParam_t;

typedef struct MI_ISP_IQ_CsaManualAttr_s
{
    MI_ISP_IQ_CsaParam_t stParaAPI;
} MI_ISP_IQ_CsaManualAttr_t;

typedef struct MI_ISP_IQ_CsaAutoAttr_s
{
    MI_ISP_IQ_CsaParam_t stParaAPI[MI_ISP_AUTO_NUM];
} MI_ISP_IQ_CsaAutoAttr_t;

typedef struct MI_ISP_IQ_CsaType_s
{
    MI_ISP_IQ_Bool_e          bEnable;  // 0 ~ 1
    MI_ISP_IQ_OpType_e        enOpType; // M_AUTO ~ (M_MODMAX-1)
    MI_ISP_IQ_CsaAutoAttr_t   stAuto;
    MI_ISP_IQ_CsaManualAttr_t stManual;
} MI_ISP_IQ_CsaType_t;

typedef struct MI_ISP_IQ_YclpfParam_s
{
    MI_ISP_IQ_Bool_e  bLpfEn;                //0 ~ 1
    MI_ISP_IQ_Bool_e  bLpfdiEn;              //0 ~ 1
...
{
    MI_ISP_IQ_Bool_e bEnable;            //0 ~ 1
    MI_ISP_IQ_YclpfManualAttr_t stManual;
