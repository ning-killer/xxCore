...
MI_S32 ST_OSD_Init(MI_S32 s32ChipId)
{
    ...
    if (g_bInit == FALSE)
    {
        ...
        MI_RGN_Init(s32ChipId, &g_stPaletteTable);
    }
...
{
    if (hHandle < 0 || hHandle >= MAX_RGN_NUM) return -1;
    ExecFunc(MI_RGN_Create(s32ChipId, hHandle, pstRegion), MI_RGN_OK);
...
MI_S32 ST_OSD_Destroy(MI_S32 s32ChipId, MI_RGN_HANDLE hHandle)
{
    ...
    ExecFunc(MI_RGN_Destroy(s32ChipId, hHandle), MI_RGN_OK);
...
MI_S32 ST_OSD_DrawPoint(MI_RGN_HANDLE hHandle, ST_Point_T stPoint, MI_U32 u32Color)
{
	...
	//g_stRgnInfo[hHandle].ePixelFmt = E_MI_RGN_PIXEL_FORMAT_I4;
...
MI_S32 ST_OSD_DrawLine(MI_RGN_HANDLE hHandle, ST_Point_T stPoint0, ST_Point_T stPoint1, MI_U8 u8BorderWidth, MI_U32 u32Color)
{
    ...
    //printf("point0(%d,%d), point1(%d,%d) color = %u\n", stPoint0.u32X, stPoint0.u32Y, stPoint1.u32X, stPoint1.u32Y, u32Color);
	...
	if(stPoint0.u32X == g_stRgnInfo[hHandle].stCanvasInfo.stSize.u32Width)
		stPoint0.u32X -= 2;
	if(stPoint0.u32Y == g_stRgnInfo[hHandle].stCanvasInfo.stSize.u32Height)
		stPoint0.u32Y -= 2;

	if(stPoint1.u32X == g_stRgnInfo[hHandle].stCanvasInfo.stSize.u32Width)
		stPoint1.u32X -= 2;
	if(stPoint1.u32Y == g_stRgnInfo[hHandle].stCanvasInfo.stSize.u32Height)
		stPoint1.u32Y -= 2;

	
#endif
	//printf("[change]point0(%d,%d), point1(%d,%d)\n", stPoint0.u32X, stPoint0.u32Y, stPoint1.u32X, stPoint1.u32Y);
	...
	printf("dx x(%3.0f,%d), dy y(%3.0f,%d)\n", dx, x, dy, y);
    // draw v line
    if (dx == 0)
    {
    ...
    int base_x = stPoint0.u32X;
    int base_y = stPoint0.u32Y;
    for (i = 0; i <= dx; i ++)
    {
        do
        {
            stPoint.u32X = base_x ;
            stPoint.u32Y = base_y ;
			//printf("[shawn debug k = %3.0f  e = %3.0f] x = %d , y = %d \n", k, e, stPoint.u32X, stPoint.u32Y);
            if(e >= 1)
            {
                if(stPoint0.u32Y > stPoint1.u32Y)
                    base_y --;
                else
                    base_y ++;
                e--;
            }
            for (int w = 0; w < u8BorderWidth; w++)
            {
                stPoint.u32Y += w;
                ST_OSD_DrawPoint(hHandle, stPoint, u32Color);   
            }
        ...
        if(stPoint0.u32X > stPoint1.u32X)
            base_x --;
        else
            base_x ++;
...
MI_S32 ST_OSD_Update(MI_S32 s32ChipId, MI_RGN_HANDLE hHandle)
{
    ...
    memset(&g_stRgnInfo[hHandle].stCanvasInfo, 0, sizeof(MI_RGN_CanvasInfo_t));
    MI_RGN_UpdateCanvas(s32ChipId, hHandle);

    return MI_RGN_OK;
}

MI_S32 ST_OSD_GetCanvasInfo(MI_S32 s32ChipId, MI_RGN_HANDLE hHandle, MI_RGN_CanvasInfo_t** ppstCanvasInfo, MI_RGN_PixelFormat_e pixelfmt)
{
    MI_S32 sRet = MI_RGN_OK;

    ST_OSD_INIT_CHECK(hHandle);
    sRet = MI_RGN_GetCanvasInfo(s32ChipId, hHandle, &g_stRgnInfo[hHandle].stCanvasInfo);
	g_stRgnInfo[hHandle].ePixelFmt = pixelfmt;
	printf("pixel fmt  = %d\n", (int)g_stRgnInfo[hHandle].ePixelFmt);
    if (sRet == MI_RGN_OK)
    {
