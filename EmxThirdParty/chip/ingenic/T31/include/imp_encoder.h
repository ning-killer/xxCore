...
typedef struct {
  int32_t       iNumBytes;       /**< 一帧流的字节数 */
  uint32_t      uNumIntra;       /**< 帧内模式8*8编码单元个数 */
  uint32_t      uNumSkip;        /**< 变换跳过8*8编码单元个数 */
  uint32_t      uNumCU8x8;       /**< 8*8编码单元个数 */
  uint32_t      uNumCU16x16;     /**< 16*16编码单元个数 */
  uint32_t      uNumCU32x32;     /**< 32*32编码单元个数 */
  uint32_t      uNumCU64x64;     /**< 64*64编码单元个数 */
  int16_t       iSliceQP;        /**< 编码片QP */
  int16_t       iMinQP;          /**< 最小QP */
  int16_t       iMaxQP;          /**< 最大QP */
 ...
 *
 * @param[in] encChn 编码Channel号,取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] iQP    QP值,取值范围: [iMinQP, iMaxQP]
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 * @remarks 调用此API会重新设置编码下一帧QP，设置的QP在下一帧生效。
 *			调用此API需要通道已经存在。
 *
 * @attention 此API只适用于H264和h265编码channel
 */
 ...
 *
 * @param[in] encChn    编码Channel号,取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] IPDelta   I帧与其后第一个P帧QP差值
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 ...
 * @param[in] encChn       通道编号.
 * @param[in] stream       码流.
 * @param[in] frame        需要统计的帧数(GOP长度的整数倍).
 * @param[out] br          平均码率.
 *
 * @retval  >=0 && < 32    成功.
 * @retval  <0			   失败.
 *
 * @remarks   此API必须在IMP_Encoder_GetStream后使用.
 *
 * @attention 无.
 */
