...
#define AE_SNAP_LOG_BUFF_SIZE	(8*1024)
...
CVI_S32 CVI_ISP_SetAEWinStatistics(VI_PIPE ViPipe, const ISP_AE_WIN_STATISTICS_CFG_S *stAeWinCfg);
CVI_S32 CVI_ISP_GetAEWinStatistics(VI_PIPE ViPipe, ISP_AE_WIN_STATISTICS_CFG_S *stAeWinCfg);
...
CVI_S32 CVI_ISP_GetAELogPath(char *szPath);
CVI_S32 CVI_ISP_SetAELogName(const char *szName);
CVI_S32 CVI_ISP_GetAELogName(char *szName);
...
CVI_S32 CVI_ISP_GetAELogBuf(VI_PIPE ViPipe, CVI_U8 *pBuf, CVI_U32 bufSize, AE_EXPOSURE_MODE mode);
