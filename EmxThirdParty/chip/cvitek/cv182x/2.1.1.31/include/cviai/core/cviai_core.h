...
/**
 * @brief MobileDetV2 object detection, the most lightweight MobileDetV2 with 6 classes
 * detection. This model cat detect "person", "bicycle", "car", "motorbike", "bus",
 * and "truck".
 *
 * @param handle An AI SDK handle.
 * @param frame Input video frame.
...
DLL_EXPORT CVI_S32 CVI_AI_MotionDetection(const cviai_handle_t handle, VIDEO_FRAME_INFO_S *frame,
                                          cvai_object_t *objects, uint32_t threshold,
                                          double min_area);
