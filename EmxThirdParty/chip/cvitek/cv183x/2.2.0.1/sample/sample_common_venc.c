...
RETRY_GET_STREAM:
		...
		if (s32Ret != CVI_SUCCESS) {
			if (s32Ret == CVI_ERR_VENC_BUSY) {
				CVI_VENC_INFO("CVI_VENC_GetStream, VencChn Retry= %d,s32Ret = 0x%X\n",
				VencChn, s32Ret);
...
CVI_S32 SAMPLE_COMM_VENC_GetGopAttr(VENC_GOP_MODE_E enGopMode, VENC_GOP_ATTR_S *pstGopAttr)
{
	...
	default:
		CVI_VENC_ERR("not support the gop mode !\n");
		return CVI_FAILURE;
	}
	return CVI_SUCCESS;
}
		...
		VENC_GOP_ATTR_S *pstGopAttr)
{
	...
	if (enType == PT_H264) {
		...
		case 0:
			h264Entropy.u32EntropyEncModeI = 0;
			h264Entropy.u32EntropyEncModeP = 0;
			break;
		case 1:
			h264Entropy.u32EntropyEncModeI = 1;
			h264Entropy.u32EntropyEncModeP = 1;
			break;
		default:
			h264Entropy.u32EntropyEncModeI = 1;
			h264Entropy.u32EntropyEncModeP = 1;
		...
		CVI_BOOL bRcnRefShareBuf)
{
	...
	pstVencChnAttr->stVencAttr.bSingleCore = pIc->single_core;
	...
	if (pstGopAttr->enGopMode == VENC_GOPMODE_NORMALP) {
		if (pIc->s32IPQpDelta <= 51 && pIc->s32IPQpDelta >= -51)
...
CVI_S32 SAMPLE_COMM_VENC_InitVBPool(vencChnCtx *pvecc, VENC_CHN VencChnIdx)
{
	...
	//update config information into vb config
