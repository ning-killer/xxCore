...
CVI_S32 SAMPLE_AUDIO_DEBUG_LEVEL(ST_AudioUnitTestCfg *testCfg)
{
	...
	_UNUSED_VAR(testCfg);
	printf("using export cviaudio_level=0~5 ... to change audio print level\n");
	return CVI_SUCCESS;
}
...
CVI_S32 SAMPLE_AUDIO_DEBUG_RECORD(ST_AudioUnitTestCfg *testCfg)
{
	...
	_UNUSED_VAR(testCfg);
	printf("Please use sample_audio 19\n");
	return CVI_SUCCESS;
}
...
CVI_S32 SAMPLE_AUDIO_DEBUG_PLAY(ST_AudioUnitTestCfg *testCfg)
{
	...
	printf("Please use sample_audio 20 [filename]\n");
	_UNUSED_VAR(testCfg);
	return CVI_SUCCESS;
}
...
CVI_S32 SAMPLE_AUDIO_DEBUG_VQE_PLAY(ST_AudioUnitTestCfg *testCfg)
{
	...
	printf("Internal test ...not support in cusotmer mode\n");
	_UNUSED_VAR(testCfg);
	return CVI_SUCCESS;
}
...
CVI_S32 SAMPLE_COMM_AUDIO_Quick_Test(void)
{
	...
	printf("internal test for chip, please use sample_audio_internal from image/tool\n");
	return CVI_SUCCESS;
}
...
CVI_S32 SAMPLE_COMM_AUDIO_CfgAcodec_Test(void)
{
	...
	//if (stAudTestCfg.unit_test == 1) cx 2023.2.15
    {
...
CVI_VOID *SAMPLE_COMM_AUDIO_AencGetProc(CVI_VOID *parg)
{
	...
	struct timespec end;
	struct timespec now;
	//int cap_time = _get_record_time(); cx 2023.2.15
    int cap_time = 0;
	printf("cap_time = %d\n", cap_time);
	//clock_gettime(CLOCK_MONOTONIC, &now);
	end.tv_sec = now.tv_sec + cap_time + 1;
				    ...
				    AIO_ATTR_S *pstAioAttr, PAYLOAD_TYPE_E enType)
{
	...
	if (enType != PT_AAC && stAencAttr.pValue != NULL) {
		free(stAencAttr.pValue);
	}
