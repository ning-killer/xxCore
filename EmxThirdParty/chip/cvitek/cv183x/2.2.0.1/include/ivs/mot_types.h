#ifndef _CVI_IVS_MOT_TYPES_H_
#define _CVI_IVS_MOT_TYPES_H_
...
typedef struct {
  ...
  float P_beta[8];
  int P_x_idx[8];
} kalman_tracker_config_t;

typedef struct {
  float max_distance_iou;
  int max_unmatched_times_for_bbox_matching;
  kalman_filter_config_t kfilter_conf;
  kalman_tracker_config_t ktracker_conf;
} mot_config_t;
