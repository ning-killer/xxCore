...
struct DefaultDeleter {
  ...
  template <typename U> DefaultDeleter(const DefaultDeleter<U>& other) {
    ...
    static_assert(cmcc_rtc::is_convertible<U*, T*>::value,
...
//
// scoped_ptr<int, cmcc_webrtc::FreeDeleter> foo_ptr(
...
struct ShouldAbortOnSelfReset {
  template <typename U>
  static cmcc_rtc::internal::NoType Test(const typename U::AllowSelfReset*);

  template <typename U>
  static cmcc_rtc::internal::YesType Test(...);

  static const bool value =
      sizeof(Test<T>(0)) == sizeof(cmcc_rtc::internal::YesType);
};
...
class scoped_ptr_impl {
 public:
  ...
  scoped_ptr_impl(scoped_ptr_impl<U, V>* other)
    ...
    // emulation to have cmcc_webrtc::subtle::move() and cmcc_webrtc::subtle::forward()
...
// types.
template <class T, class D = cmcc_webrtc::DefaultDeleter<T> >
class scoped_ptr {
  ...
  // enabled.
  //static_assert(cmcc_webrtc::internal::IsNotRefCounted<T>::value,
 ...
 public:
  ...
  scoped_ptr(scoped_ptr<U, V>&& other)
      : impl_(&other.impl_) {
    static_assert(!cmcc_rtc::is_array<U>::value, "U cannot be an array");
  }
 ...
 private:
  typedef cmcc_webrtc::internal::scoped_ptr_impl<element_type, deleter_type>
      scoped_ptr::*Testable;
 ...
 private:
  ...
  cmcc_webrtc::internal::scoped_ptr_impl<element_type, deleter_type> impl_;
...
template <class T, class D>
void swap(cmcc_webrtc::scoped_ptr<T, D>& p1, cmcc_webrtc::scoped_ptr<T, D>& p2) {
  p1.swap(p2);
}

template <class T, class D>
bool operator==(T* p1, const cmcc_webrtc::scoped_ptr<T, D>& p2) {
  return p1 == p2.get();
}
...
template <typename T>
cmcc_webrtc::scoped_ptr<T> rtc_make_scoped_ptr(T* ptr) {
  return cmcc_webrtc::scoped_ptr<T>(ptr);
}
