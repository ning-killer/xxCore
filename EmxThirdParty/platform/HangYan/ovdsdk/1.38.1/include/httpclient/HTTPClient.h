...
// 外部功能
#define HTTP_CLIENT_FLAG_ARES               0x00000200 // Use ovd c-ares for dns
#define HTTP_CLIENT_FLAG_LITEOS             0x00000400 // Use voip liteos
#define HTTP_CLIENT_FLAG_HOST               0x00000800 // Use voip upload host
#define HTTP_CLIENT_FLAG_SPEEDUP            0x00001000 // speed up httpclient
#define HTTP_CLIENT_FLAG_SKIPCACHE          0x00002000 // httpclient skip get ip form file
...
#define ALIGN4(size) ((size & 0xfffffffc) + ((size & 3) ? 4 : 0))

typedef void   E_HTTPDebug(const char *,const char*,UINT32,char *,...); // HTTPDebug hook function
#define HTTP_DEBUG(session, fmt, ...) do {                                       \
    if(((P_HTTP_SESSION)session)->pDebug)                                            \
    {                                                                   \
        ((P_HTTP_SESSION)session)->pDebug(__func__, NULL, 0, fmt, ##__VA_ARGS__);    \
    }                                                                   \
}while(0)

//当前安防只在length为0时打印
#define HTTP_OVD_DEBUG(session, fmt, ...) do {                                       \
        if(((P_HTTP_SESSION)session)->pDebug)                                            \
        {                                                                   \
            ((P_HTTP_SESSION)session)->pDebug(__func__, NULL, 0, fmt, ##__VA_ARGS__);    \
        }                                                                   \
...
#define H_FREE     free
    ...
    typedef struct _HTTP_CONNECTION
    {
        ...
        INT32               HttpSocketfd4;         // The underling socket
        INT32               HttpSocketfd6;         // The underling socket
...
typedef int (*HTTP_GETADDRINFO)(const char *node, const char *service,
                       const struct addrinfo *hints,
                       struct addrinfo **res);
typedef void (*HTTP_FREEADDRINFO)(struct addrinfo *res);
    ...
    typedef struct _HTTP_REQUEST
    {
        ...
        HTTP_GETADDRINFO    http_getaddrinfo;
        HTTP_FREEADDRINFO   http_freeaddrinfo;
    ...
    UINT32                  HTTPClientPostChecked         (HTTP_SESSION_HANDLE pSession, CHAR *pUrl, VOID *pData, UINT32 nDataLength, UINT32 nTimeout, UINT32 nClientPort);
    UINT32                  HTTPClientCheckedSend         (HTTP_SESSION_HANDLE pSession, VOID *pData, UINT32 nDataLength, UINT32 nTimeout);
    ...
    UINT32                  HTTPSIntrnConnectionOpen       (P_HTTP_SESSION pHTTPSession);
    ...
    void                    HTTPClient_SetAppSecretSN(const char *app_secret, const char *sn);
