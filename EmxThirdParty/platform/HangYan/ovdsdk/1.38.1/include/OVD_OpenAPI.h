...
typedef struct
{
    ...
    *【回调】获取设备Gps信息
    *
    * @description：OVC获取行车记录仪坐标信息时调用，厂商返回响应坐标信息
    *
    ...
    */
    ...
    /**
      *【回调】重启设备
      *
      * @description：sdk通知厂商重启
      * @param[in]: reason sdk同步的重启原因，目前主要包含自动维护重启，软看门狗重启，远程重启
      * @others：厂商收到此回调了，务必同步执行重启动作，不能跟其他业务进行耦合
      *
      * @return
      * - = 0:  成功
      * - < 0:  失败
	...
	* - < 0:  失败
    */
	OVD_int32 (*OVD_ROBOTCmd)(ovd_robot_content_t* robotcmd,void* data);
	...
	* @return
 	...
 	* - < 0:  失败
	*/
    OVD_int32 (*OVD_ForceIFrame)(OVD_int32 channel, OVDCodeStream code_stream);
    ...
    */
    ...
    /**
    * 【回调】OVC通知OVD播放音乐/音频时调用,暂不启用
    * @description：OVD去url下载音乐内容，并在channel的音箱上播放
    *
	...
	*/
    ...
    /**
	*【回调】同步macHash信息（功能弃用，此接口弃用）
	*
	* @description：同步macHash信息给厂商
	*
	* @param[in] macHash 厂商需将该信息固化到设办理，且重启及恢复出产设置时，均不能擦除，下次初始化时，厂商初始化sdk时需回填该信息，否则会被强制下线
	*
	* @return
	* - = 0:  成功
	* - < 0:  失败
	*/
    OVD_int32 (*OVD_syncmachash)(OVD_char* in_machash);
    ...
    * - < 0:  失败
    */
    OVD_int32(*OVD_GetDevRunningInfo)(ovd_probe_devrunning_info_e info_e, void *data);
	 ...
	 * - 其他值：失败
	 */
	OVD_int32(*ovd_running_status_cb)(ovd_probe_exception_type_e status, OVD_int32 channel_id, void* data, void* user_data);
    ...
    * - < 0:  失败
    */
    OVD_int32(*OVD_homibot_set_config)(OVD_char *cmd, OVD_char *config, OVD_int32 size);
    ...
    * - > 0   上报数据大小
    */
    OVD_int32(*OVD_homibot_get_config)(OVD_char *cmd, OVD_char *buffer, OVD_int32 size);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_CapInit(OVDCapInfo *capinfo);
 ...
 */
extern OVD_EXPORT OVD_int32 OVD_Init( IN OVDCapInfo *capinfo,  IN OVDClientParam *clientParam, IN OVDLogParam *logParam, IN ovd_reboot_param_t *reboot_info,
                           IN OVD_CallBackFunList *callBackFunList, IN OVD_char *jsonParam);

/**
* sdk反初始化接口
* @return
*   OVD_RET_SUCCESS; 0 反初始化成功
*    其他：             失败
*/
extern OVD_EXPORT OVD_int32 OVD_deint();
/**
 * 功能：设备获取SDK版本号
 */
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_GetSDKVersion(char *version, int version_len);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_InstallChannel(int channel, OVD_ChannelCap_t *cap);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_UninstallChannel(int channel);
...
extern OVD_EXPORT OVD_void* OVD_SoundWaveInit(OVD_int32 sampleRate,OVD_int32 bitWidth);
 ...
 * - = 101: sdk未集成声波识别模块
 */
extern OVD_EXPORT OVD_int32 OVD_SoundWaveStart(OVD_void *recognizer,RecognizerStart starFunc,RecognizerFinish endFunc);
 ...
 * - 失败:  < 0
 */
extern OVD_EXPORT OVD_int32 OVD_SoundWaveWriteData(OVD_void *recognizer,const OVD_void *data,OVD_uint64 len);
 ...
 * - = 101: sdk未集成声波识别模块
 */
extern OVD_EXPORT OVD_int32 OVD_SoundWaveStop(OVD_void *recognizer);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_QRString_init();
 ...
 */
extern OVD_EXPORT OVD_int32 OVD_QRString(OVDXXSSIDWiFiInfo *info, OVD_uchar *qr_str,OVD_uint32 qr_len);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_ApConf_start(OVDXXSSIDWiFiInfo *info);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_DeviceBindInfo(OVD_char *bindid);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_AVPushStart(OVD_int32 channel, OVDCodeStream code_stream, OVDVideoDataFormat *videoinfo, OVDAudioDataFormat *audeoinfo);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_ServiceStart();
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_ServiceStop();
 ...
 * - =  1:  服务器连接异常但告警被缓存
 */
extern OVD_EXPORT OVD_int32 OVD_AlarmInfoStart(OVDUpLoadAlarmInfo *alarmInfo);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_AlarmInfoEnd(OVD_int32 channel, OVDAlarmType alarmType, OVD_uint64 endTimeStamp);


/**
 * 机器人告警信息上报
 *
 * @description：如果告警上报失败，SDK不会缓存该告警，需要设备侧再次发送告警
 *
 ...
 * - = -1:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_HomibotEventNotify (OVDRobotAlarmInfo *alarmInfo);
 ...
 *
 * @param[in] channel     通道号,如果是摄像头产品，则通道号为0，如果是nvr等产品，channel则为前端摄像头的索引
 * @param[in] code_stream 码流索引,如果支持主子码流，OVD_HIGH_STREAM为主码流，OVD_LOW_STREAM为子码流，不支持多码流，默认为OVD_HIGH_STREAM
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_AVParamModify(OVD_int32 channel, OVDCodeStream code_stream, OVDVideoDataFormat *videoinfo,OVDAudioDataFormat *audeoinfo);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_AVPushData(OVD_int32 channel, OVD_int32 code_stream, OVDContentType contentType, OVD_bool isIFrame, OVD_uchar *contentData,
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_AVPushEnd(OVD_int32 channel, OVD_int32 code_stream);

/**
 * 行车记录仪音视频内容推送接口，且云存开始录制
 * @description：行车记录仪音视频内容推送接口，且云存开始录制
 * @others：debug_ts置为1，日志将输入每帧参数打印
 * @param[in] channel       通道号,如果是摄像头产品，则通道号为0
 * @param[in] alarmType   告警类型
 * @param[in] contentType   准备传送的内容，详见枚举值OVD_ContentType 音频、视频
 * @param[in] isIFrame      是否是I帧  0：不是 1：是
 * @param[in] contentData   发送数据的首字节指针
 * @param[in] videoDataLen  本次发送数据的长度
 * @param[in] timestamp     该帧时间戳(ms)，厂商需传入绝对时间戳， * @return
 * - = 0:  成功
 * - < 0:  失败
*/
extern OVD_EXPORT OVD_int32 OVD_driving_avpush_data(OVD_int32 channel, OVDAlarmType alarmType, OVDContentType contentType, OVD_bool isIFrame, OVD_uchar *contentData,OVD_int32 dataLen, OVD_uint64 timestamp);

/**
 * 行车记录仪音视频内容结束推送接口，且云存停止录制
 * @description：行车记录仪音视频内容结束推送接口，且云存停止录制
 * @param[in] channel     通道号
 * @param[in] alarmType   告警类型
 * @return
 * - = 0:  成功
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_driving_avpush_end(OVD_int32 channel, OVDAlarmType alarmType);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_SendRecordAVContent(OVD_int32 channel, OVDContentType contentType, OVD_bool isIFrame, OVD_void *contentData,
        OVD_int32 dataLen, OVD_uint64 timestamp);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_RecordAVContentSendOver(OVD_int32 channel);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_LogDone(OVD_char* trans_id, OVD_int32 result, OVD_char* start, OVD_char *end, OVD_char *url,OVD_uint64 size);
 ...
 * - < 0:  失败
 */
extern OVD_EXPORT OVD_int32 OVD_BatteryChange(OVD_int32 battery);
 ...
 * @return 0代表正常， -1代表异常
 */
extern OVD_EXPORT OVD_int32 OVD_GetHServerInfo(ovd_hserverinfo_t *hserverinfo);
...
extern OVD_EXPORT OVD_int32 OVD_register_thirdparty_gettimeofday(OVD_callback_thirdparty_gettimeofday func);
 ...
 * - < 0:  失败
 */
OVD_EXPORT OVD_int32 ovd_probe_start();
 ...
 * - < 0:  失败
 */
OVD_EXPORT OVD_int32 ovd_probe_paused();


/**
 * SDK提供日志打印
 *
 * @description：最多打印1024字节内容
 *
 * @param[in] level    日志等级，详见OVDLogLevel
 * @param[in] log      日志内容
 *
 * @return
 * void
 */
OVD_EXPORT OVD_void OVD_Printf(OVDLogLevel level,OVD_char *content, ...);

/**
 * SDK内部日志级别调整
 *
 * @param[in] value    日志等级，详见OVDLogLevel
 *
 * @return
 * - = 0:  成功
 * - < 0:  失败
 */
OVD_EXPORT OVD_int32 OVD_setloglevel(OVDLogLevel value);
 ...
 * - < 0:  失败
 */
OVD_EXPORT OVD_int32 OVD_updateOVDstate(enum ovd_state state);
 ...
 * - < 0:  失败
 */
OVD_EXPORT OVD_int32 OVD_updatechannelstate(OVD_int32 channel, enum ovd_channel_state channelstate);
 ...
 * - < 0:  失败
 */
OVD_EXPORT OVD_int32 OVD_StartLiveCapture(OVD_char *filepath, OVD_char* filepre);
 ...
 * - < 0:  失败
 */
OVD_EXPORT OVD_int32 OVD_StartLiveCaptureEx(OVD_int32 channel, OVDCodeStream stream, OVD_char *filepath, OVD_char* filepre);
 ...
 * - < 0:  失败
 */
OVD_EXPORT OVD_int32 OVD_StopLiveCaptureEx(OVD_int32 channel, OVDCodeStream stream);
 ...
 * - < 0:  失败
 */
OVD_EXPORT OVD_int32 OVD_StartRecordCapture(OVD_char *filepath, OVD_char* filepre);
 ...
 * 设置卡状态
 *
 * @description：调用此接口用于设置卡录像的SD卡状态，暂不启用
 *
 * @param[in] channel   通道号
 * @param[in] info      卡状态信息
 *
 ...
 * - < 0:  失败
 */
OVD_EXPORT OVD_int32 ovd_set_storage_info(OVD_int32 channel, ovd_local_storage_info_t *info);
/**
 * 上报异常信息
 * @param[in] event_info   异常上报，详情见结构体ovd_probe_except_info_t
 *
 * @return
 * - = 0:  成功
 * - < 0:  失败
 */
OVD_EXPORT OVD_int32 ovd_probe_exception_post(ovd_probe_except_info_t event_info);

/**
 * 恢复出厂设置
 * @description： 当设备整机恢复出厂设置时，SDK保存在设备上的数据文件也要恢复出厂设置，由设备厂商调用该接口重置SDK数据文件
 *
 * @return
 * - = 0:  成功
 * - < 0:  失败
