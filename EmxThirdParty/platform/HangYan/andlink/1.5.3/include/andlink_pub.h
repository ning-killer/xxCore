...
typedef struct 
{
	...
	char *deviceType;          // 产品类型ID,即和家亲生态门户上创建完产品后,生成的 产品ID;
	char *deviceMac; 		   // 厂商可以填mac或sn,即设备唯一标识
	char *andlinkToken; 	   // 平台对接 非必选; 杭研andlink IOT云平台秘钥;
	char *productToken; 	   // 平台对接 非必选;设备一型一密预置秘钥;
	char *firmWareVersion;	   // 设备固件版本号 
	char *softWareVersion;     // 设备软件版本号 
	char *dmKey;			   // V1.4.3及之后版本,DM迁移到andlink平台后,废弃,此字段可填NULL;
	char *cfgPath;             // andlink配置文件存储路径,此路径需要可读写,断电不丢失;
	char *extInfo;             // andlink所需的扩展信息
}adl_dev_attr_t;
...
typedef struct 
{
	...
	// 必须实现;注意入参可能其中之一为空的情况,sdk需要哪个参数,哪一项才会传入地址,可参考示例代码
	// 获取设备IP;sdk以此判断设备是否联网;SDK调用时,两个参数分配的缓存长度为40B
...
// 按需调用,平台对接的设备不会调用;本地局域网数据上报,默认上报到中国移动生态的路由器;
int local_devDataReport(char *childDevId, char *eventType, char *seqId, char *data, int dataLen);
...
int adl_setDevFunc(unsigned int funcPos);
#endif
