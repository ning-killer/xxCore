...
typedef struct 
{
	...
	// 读配置项的接口; 举例getCfg("deviceId", a, sizeof(a)-1);实现可选;若不实现此接口,sdk使用内部实现的读配置项接口
	int (*getCfg_callback)(char *item, char *value, int bufsize);

	// 写配置项的接口;举例SetCfg("deviceId","CMCC-10086-666677778888");实现可选;若不实现此接口,sdk使用内部实现的写s配置项接口
	int (*setCfg_callback)(char *item, char *value);

	// DM信息获取;add by 21.02.19
...
// sdk 启动(sdk中devAttr,devCbs增加字段,不支持向上兼容,故sdk此接口变化导致的升级,应用程序也需要跟着升级)
...
//设置工作环境，默认是线上环境，可以通过此接口设置为测试环境
int setTestEnvFlag(int flag);
int getTestEnvFlag();



// 设置andlink单个日志文件最大阈值,默认0x80000,即500KB 
...
//查询平台对接DM SDK版本号,仅针对cloud access的设备
...
//禁止andlink的某些功能,funcID可以是ADL_FUNCS_e中元素的组合
