...
typedef enum {
    E_CS_SERVICE_NONE    = 0,
    E_CS_SERVICE_VIDEO   = 1,
    E_CS_SERVICE_PICTURE = 4,
} cs_service_type_t;
...
typedef enum {
    ...
    /* 固件升级失败 */
    E_IOT_EVENT_FIRMWARE_UPDATE_FAILE = 119,
    /* 缩时录影（行车记录仪）*/
...
typedef enum {
    ...
    E_IOT_CAR_RUN = 145,
    /* 汽车点火状态ACC(OFF),防盗模式 */
    E_IOT_EVENT_ACC_OFF1 = 156,
    /* 汽车点火状态ACC(OFF),缩时录像模式 */
    E_IOT_EVENT_ACC_OFF2 = 157,
    /* 汽车点火状态ACC(OFF) */
    E_IOT_EVENT_ACC_OFF3 = 158,
