...
typedef struct stru_ZJ_AIPIC_NODE
{
    ...
    unsigned int  uiSimilarity;             // 相似度 必填 ZJ_SetAiPicEvent使用
    unsigned char aucCarNum[16];            // 车牌号   ZJ_SetAiPicEventEx2使用
    unsigned char aucLabelID[64];           // AI人脸布控图片ID pFunAddSingleAiPic回调接口的pucPicID参数  ZJ_SetAiPicEvent使用
...
{
    unsigned char  ucPicPath[64];             // AI事件图片本地文件路径 UID_AI_yyyymmddhhmmss.jpg
    unsigned char  ucVideoPath[64];           // AI事件视频本地文件路径 UID_AI_yyyymmddhhmmss.ps
